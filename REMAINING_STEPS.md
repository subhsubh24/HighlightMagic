...
### 0e. AppStoreMetadata.swift honesty rewrite (HIGH PRIORITY — pre-submission)

**LOOP-DEFERRED (loop CAN build, but iOS-unverifiable + needs feature-by-feature verification).**
`Sources/Utilities/AppStoreMetadata.swift` is the reference metadata the owner copies into App Store
Connect, and its **App Review Notes** (read by Apple's reviewer) currently state "music, sound effects,
and voiceover are generated by ElevenLabs; animated photo clips and intro/outro video cards are generated
by AtlasCloud (Kling)" — **all hard-disabled/non-functional in v1** (per 0d). The `description`,
`whatsNew`, screenshot specs, and free/pro feature lists also tout "14 royalty-free music tracks / 8
styles / 6 filters" etc. This must be corrected to match the shipped binary BEFORE submission (false
App Review Notes are a rejection risk; false store description is an FTC/accuracy risk). Deferred Run 34
because it is a large, interwoven set on an iOS file the loop cannot compile-verify on Linux, and several
non-audio counts (style-template count, premium LUTs/particle overlays, "7-pass AI") need independent
verification before rewriting. Do a careful, string-only, feature-verified pass in a future run (gated by
the `ios` check; abandon if it fails twice), or the owner reviews/edits this at submission time.

**What the owner can do to unblock**: nothing required now (loop work), but MUST verify these App Review
Notes + description reflect the actually-shipped features before submitting.
