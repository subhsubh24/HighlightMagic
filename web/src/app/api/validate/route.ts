...
17. **Human feel test** — The #1 sign of AI editing is UNIFORMITY. Compare parameter values across all clips:
   ROBOTIC example: every clip has transDur=0.3, every velocity uses the same preset, every caption uses pop animation, SFX on every cut, identical clipAudioVolume values.
   HUMAN example: clip 1 has transDur=0.22 with slide caption, clip 2 has transDur=0.45 with no caption, clip 3 is a hard cut with flicker and no SFX — variation that reflects different moments.
   ...
   **Parameter presence variation**: Don't just compare values — check which parameters are PRESENT vs ABSENT across clips. A human editor wouldn't set beatPulseIntensity, captionGlowColor, or transitionParams on every single clip. Some clips should have these fields omitted entirely (using defaults), not just set to different values. If every clip has the same set of optional parameters populated, that's a machine fingerprint even if the values differ.
