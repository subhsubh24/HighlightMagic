...
- VALIDATION COMPLETENESS gate is LIVE (2026-06-29, ROADMAP G6): every external service/secret must be registered in `web/src/lib/validation-manifest.ts` with a validation mode. `validation-manifest.test.ts` runs in the REQUIRED `web` check and FAILS if any `process.env.*` is unregistered → a NEW service HARD-BLOCKS its PR until you register it + build its validation (keyless contract test for `mock`; eval + OWNER_ACTION `validation-eval-keys` for `live-eval`). Don't tick a `live-eval` capability done until the gated `.github/workflows/live-eval.yml` (cadence/manual; Anthropic detect live now, ElevenLabs/AtlasCloud as G3 evals land; skips+warns without keys) has run green. Owner is funding Anthropic+ElevenLabs+AtlasCloud eval keys (GH Actions secrets). See docs/ci/VALIDATION.md. Don't re-propose this.
