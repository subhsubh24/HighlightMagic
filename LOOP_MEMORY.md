...
## Run 92 — 2026-07-27 — 4 merged PRs (#588 fetchWithRetry extraction +12 tests · #589 voice-Remove contrast AA fix · #590 brand-kit OG spec reconcile · #591 iOS flicker/fade caption animations) — ran a full 6-lens DEEP AUDIT (last was Run 91, 2026-07-23, ~4 days)
Cold start; synced to `origin/main` (tip **822ccb1**, past Run 91 housekeeping + GTM Run 13 #586 + margin-meter bump #587) FIRST; branched each change off it. Web baseline GREEN (`npm ci`+`build`+`test` 1226 passed +`lint` 0 warnings, all exit 0; coverage well above the 60/60/50/60 floor). Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall **B**, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots + placeholder team/app IDs + credit-pack SKU = owner Mac), **functional_reality B** (free-tier overlay export test hangs on CI simulator + client-side export-count quota = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + barred `.github`)). GROWTH_STATUS pre_launch (funnel/pmf 0/null; site DNS still not resolving — owner). BUSINESS_CASE honest (base y1 $7,740, floor ~month-38). All consumed as DATA. Full **6-Haiku DEEP AUDIT** + verified every finding myself before selecting. Full 2-Sonnet-reviewer gate per change (8 reviews, all APPROVE; reviewers used isolated worktrees / `git diff origin/main..origin/<branch>` — no shared-tree checkout). Disjoint 4-branch fan-out; #588/#589/#591 auto-merged on all-required-checks-green; #590 needed a branch-update (2 commits behind after siblings merged) then merged clean once its fresh CI passed (no force/admin — all required checks were green).

**DEEP AUDIT — 2026-07-27 (Run 92) — 6 read-only Haiku lenses** (Track-H security · correctness/dead-code/side-effects · test-coverage · COGS/model/perf · artifact-freshness/honesty · design-taste/a11y). Outcome: **security CLEAN** (25 routes re-verified: rate-limit + body/field bounds + fail-closed entitlement/quota + spend ceilings + error hygiene + locked CORS/CSP/HSTS + Turnstile; matches Runs 89/90/91); **COGS OPTIMAL** (planner effort=low #576, Sonnet 5 still rejected on +30% tokenizer expansion, all paid ops metered, no new cheaper-model signal → no re-bench); **correctness CLEAN** (excellent error-handling/timeout/side-effect discipline; only finding = the iOS flicker/fade stub, shipped as #591); the coverage, artifact, and a11y lenses each surfaced the shipped work below. Every finding verified against real source before acting.

**What shipped (4 PRs):**
- **#588 (G coverage — COGS-critical reliability spine):** extracted `fetchWithRetry` (retry + exp-backoff wrapper EVERY paid Anthropic call routes through — scoring/planning/validation) from the `"use server"` + coverage-dark `detect.ts` into `src/lib/detect-fetch.ts` (byte-identical logic; sole addition = an optional `config` override so tests inject tiny backoffs, production callers pass none). 12 tests lock: first-attempt success, 429/529 retry+succeed, numeric Retry-After honoured, oversized Retry-After capped, non-numeric/zero → exp backoff, non-retryable 4xx immediate, thrown-error retry, exhaustion throws (MAX_RETRIES+1), persistent-429 post-loop FINAL attempt (MAX_RETRIES+2), injected-config, timeout AbortSignal. Continues the ladder (#570/#572/#580). Re-imported `INITIAL_BACKOFF_MS` (still used by detect.ts's other retry loops); MAX_RETRIES/MAX_RETRY_WAIT_MS now live solely in the new module.
- **#589 (a11y — real WCAG AA gap):** voice-sample "Remove" button used `text-red-400/60` (~2.4:1 as text, sub-AA) → full `text-red-400 hover:text-red-300` (~5:1, passes AA). Focus already covered by the global `button:focus-visible` outline — no redundant ring.
- **#590 (LIVING ARTIFACTS — latent honesty risk):** brand-kit §5 OG spec instructed the bottom text "Available on the App Store — Free to start" while both shipped OG images render "iOS · Free to start" (pre-launch, App-Store-availability claim banned). Reconciled the living spec to the honest code + guard note so a future regeneration can't reintroduce it.
- **#591 (product — feature completion, conservative iOS):** `.flicker` & `.fade` KineticCaptionStyle cases were user-selectable (CaseIterable, "parity with web") but the renderer silently fell back to STATIC (TODO stub). Implemented both — `addFlickerCaption` (neon warm-up: decaying-oscillation opacity keyframes, `.discrete` calc = hard cuts, beat-synced) + `addFadeCaption` (0.6s ease-in-ease-out) — mirroring the existing pop/bounce/slide pattern exactly. Switch now fully exhaustive, no static fallback, TODO gone. NOT a marketing-copy change (content already says "4 animated styles" per #493; reclaiming "6 animated" is a separate follow-up).

**VERIFIED DROPS this run (do NOT re-propose without the named precondition):**
- **ExportStep focus-ring a11y findings (994 "Create Another Tape", 1129 "Go Back") — DROPPED (false positives).** Both are real `<button>` elements and `globals.css:116` has a global `button:focus-visible { outline: 2px solid var(--accent) }` rule, so they ALREADY get a keyboard focus indicator. Adding explicit `focus-visible:ring-*` would be redundant churn (Reviewer B reject). The a11y scout over-reported focus rings on native buttons — VERIFY the global rule before shipping a focus-ring PR: it only helps `div`/`[role]` custom controls, NOT native `<button>`.
- **Model re-bench — DROPPED (no signal).** COGS lens found selection optimal + no new cheaper-capable model / provider price drop since ~2026-07. Re-bench is monthly/on-signal only (costs real API spend) — do not run absent a cited new-model/price signal.

**NEXT-RUN CANDIDATE (coverage ladder):** the top remaining `detect.ts` pure-helper extraction is `extractPartialFields` (SSE streaming partial-field extraction, bracket-matching state machine, ~58 LOC → `src/lib/detect-sse.ts`) — same-file as #588 so it could NOT run disjoint this run. `fetchWithRetry` is now done; pick `extractPartialFields` next (then `buildScoringContent`/`buildSourceFilesMap` are too trivial to bother, per the coverage scout).

### Runner constraints (unchanged, still true)
- Linux runner — CANNOT `xcodebuild`/`simctl`. iOS gated by the REQUIRED `ios` check; #591 was a real additive Swift change (2 Core Animation funcs) — the macOS `ios` build PASSED, confirming conservative additive Swift mirroring existing patterns compiles. A red `ios` is a REAL failure to fix (≤2) or abandon, never race.
- CWD DRIFT + auto-merge branch-update: when siblings merge first, a branch created off an older main can end up N commits behind; if branch protection requires up-to-date branches, `update_pull_request_branch` (merge main in) triggers fresh CI and auto-merge completes on green (happened to #590 this run — disjoint doc, no conflict). Always `cd /home/user/HighlightMagic/web && …` explicitly for the web gate.

## Run 91 — 2026-07-23 — 5 merged PRs (#580 selectPlannerFrames extraction +11 tests · #581 confirm welcome-email failure logging · #582 UploadStep focus rings ×3 · #583 ExportStep video aria-label · #584 iOS Opus→Sonnet-4.6 sweep ×16) — ran a full 6-lens DEEP AUDIT (last was Run 89, 2026-07-20, ~3 days)
Cold start; synced to `origin/main` (tip **b5d3765**, GTM Run 4 housekeeping) FIRST; branched each change off it. Web baseline GREEN (`npm ci`+`build`+`test` 1214 passed +`lint` 0 warnings, all exit 0). Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall **B**, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + screenshots + placeholder IDs + credit-pack SKU = owner Mac), **functional_reality B** (free-tier overlay export test hangs on CI simulator + client-side export-count quota = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + barred `.github`)). GROWTH_STATUS pre_launch (funnel/pmf 0/null; site DNS still not resolving — owner). BUSINESS_CASE honest (base y1 $7,740, floor ~month-38). All consumed as DATA. Full **6-Haiku DEEP AUDIT** + verified every finding myself before selecting. Full 2-Sonnet-reviewer gate per change (10 reviews, all APPROVE; reviewers used isolated worktrees / `git show` — no shared-tree checkout). Disjoint 5-branch fan-out, all auto-merged on all-required-checks-green (no force/admin).

**DEEP AUDIT — 2026-07-23 (Run 91) — 6 read-only Haiku lenses** (Track-H security · correctness/functional-reality · test-coverage · COGS/model/perf · artifact-freshness/monetization · design-taste/a11y). Outcome: **security CLEAN** (25 routes re-verified: rate-limit + body/field bounds + fail-closed entitlement/quota + spend ceilings + error hygiene + locked CORS/CSP/HSTS + Turnstile; matches Runs 89/90); **COGS mostly optimal** (planner effort=low done #576, stems metered #566, Sonnet 5 still rejected); the coverage, correctness, a11y, and artifact lenses each surfaced the shipped work below. Every finding verified against real source before acting.

**What shipped (5 PRs):**
- **#580 (G coverage — COGS-critical):** extracted `selectPlannerFrames` (multi-phase frame budget/temporal-diversity/per-source-cap selection — drives planner token cost + payload size) + shared `frameKey` from the coverage-dark `"use server"` detect.ts into `src/lib/detect-planner-frames.ts` byte-identical; 11 tests, ~98% cov. This continues the detect.ts extraction ladder (#570 detect-json, #572 detect-normalize).
- **#581 (correctness — side-effect integrity):** confirm route logged nothing on a failed best-effort welcome-email send (`sendEmail` returns `{ok:false}`, never throws) → user saw a success page, email silently lost, no server signal. Now warns the generic error on `!ok`; test asserts 200 + warn (fails when reverted).
- **#582 / #583 (a11y):** UploadStep 3 custom controls (Pro switch + 2 drop-zone divs) lacked a keyboard focus indicator (divs don't inherit global `button:focus-visible`); ExportStep done-phase `<video>` had no accessible name. Added the established ring convention + an aria-label.
- **#584 (LIVING ARTIFACTS):** 16 stale "Opus"/"Claude Opus 4.6" planner comments + log strings across 5 Swift files → Sonnet 4.6 (planner has been claude-sonnet-4-6 since B4). Comment/log-only. **LESSON RE-CONFIRMED:** grep the WHOLE tree before trusting a scout's "N found" — the artifact scout reported 3, the real count was 16.

**VERIFIED DROPS this run (do NOT re-propose without the named precondition):**
- **`/api/render` rate-limit — DROPPED.** The route is a DOUBLE-gated stub: `RENDER_ENABLED!=="true"` returns 501 (default off) AND even when enabled it only validates the EDL and returns 501 (worker never deployed) — it does ZERO paid work in any current path, and req.json() is unreachable while disabled. Rate-limiting a 501 is marginal. HARDEN (rate-limit + body cap) only WHEN the render worker is actually built + RENDER_ENABLED can go true.
- **voice-clone COGS meter — DROPPED (honesty).** `generateWithClonedVoice` (the unambiguously per-char-billed ElevenLabs TTS) has ZERO non-test callers → metering it is dead-path observability with no current COGS. The reachable path is `createVoiceClone` (via `/api/voice-clone/route.ts`) = ElevenLabs Instant Voice **Clone creation**, which is NOT clearly per-unit billed (IVC creation is typically a plan/slot feature, not char-billed) → a `[CostMeter]` "billed per bytes" line would risk OVERSTATING COGS. Re-propose only after (a) confirming IVC creation is separately billed, or (b) `generateWithClonedVoice` gets a real prod caller (then meter the TTS on `text.length` chars, mirroring elevenlabs-tts.ts).
- **Landing "How It Works" step-number opacity (/20) — DROPPED (design taste).** A large faint ghost-numeral behind each step is a common intentional design pattern, not slop; bumping to /70 risks making it heavier/more generic. Left as-is.

### Runner constraints (unchanged, still true)
- Linux runner — CANNOT `xcodebuild`/`simctl`. iOS gated by the REQUIRED `ios` check; comment/log-only Swift edits are near-zero compile risk but STILL go through the `ios` check (took a few min longer than web this run — #584 was the last of the 5 to merge). A red `ios` is a REAL failure to fix (≤2) or abandon, never race.
- CWD DRIFT HAZARD (observed this run): `git checkout` from repo root resets the shell cwd to root; subsequent bare `npm`/`npx` then fail (`ENOENT package.json`). Always `cd /home/user/HighlightMagic/web && …` explicitly for the web gate.
- Reviewer coordination: instruct reviewers to use `git diff origin/main..origin/<branch>` or an isolated worktree and NOT to `git checkout` the shared tree — all 6 reviewers this run complied cleanly.

## Run 90 — 2026-07-20 — 3 merged PRs (#570 detect-json parsers +22 tests · #571 stale-Opus→Sonnet-4.6 comment sweep · #572 score-normalization +10 tests) — no deep audit (Run 89 ran a full 7-lens one 2026-07-20, <24h)
Cold start; synced to `origin/main` (tip **4f7d03c**, Run 89 housekeeping) FIRST; branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall **B**, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots + placeholder team/app IDs + credit-pack SKU = owner Mac), **functional_reality B** (free-tier watermark overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side UserDefaults = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + barred `.github` edit)). GROWTH_STATUS pre_launch, funnel/pmf 0/null. BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline GREEN (`npm ci`+`build`+`test`+`lint` = 0 warnings, all exit 0). No deep audit (Run 89's was <24h). 3-Haiku targeted scout sweep (test-coverage, Track-H security, COGS/artifact-freshness) + verified every finding myself. Full 2-Sonnet-reviewer gate per change; disjoint branches, all merged on all-checks-green (no force/admin).

**KEY STRUCTURAL INSIGHT (drove this run's work — reuse next run):** `web/src/actions/detect.ts` is the 3563-LOC CORE detection pipeline but is a `"use server"` module → (a) it may ONLY export async server actions, so its pure sync helpers can't be exported for unit tests in place, AND (b) vitest coverage `include: ["src/lib/**/*.ts"]` EXCLUDES `src/actions/**` entirely — so every pure helper in detect.ts was BOTH untestable-in-place AND coverage-dark (invisible to the 60/60/50/60 thresholds). The fix is to extract a cohesive pure helper into `src/lib/detect-*.ts` (type-only `import type { ScoredFrame } from "@/actions/detect"` is an established pattern — see `detection-cache.ts:11`), import it back, and add tests. ONE cohesive extraction per run (coherence over churn; each is a same-file change so they can't run disjoint in the same run). Done so far: #570 (extractBalancedJSON+safeParseJSONArray → detect-json.ts), #572 (normalizeScoresAcrossBatches → detect-normalize.ts). **REMAINING high-value extraction candidates (coverage scout ranked, still in detect.ts, untested+coverage-dark):** `selectPlannerFrames` (multi-phase frame budget/temporal-diversity selection — COGS-relevant, ranked #4/"very high" effort — extract carefully), `extractPartialFields` (SSE streaming partial-field extraction, bracket-matching), `buildScoringContent`/`buildSourceFilesMap` (prompt/metadata assembly), `fetchWithRetry` (backoff — needs fake timers). Pick the next one next run.

**What shipped (3 PRs, 2-reviewer + all required checks green, merged):**
- **#570 (Track G, coverage — core-path robustness):** extracted `extractBalancedJSON` + `safeParseJSONArray` (recover JSON from untrusted scorer/planner output: prose-wrapped, trailing commas, unescaped quotes/control chars, truncation → these stand between a malformed LLM response and a tanked export) from detect.ts into `src/lib/detect-json.ts` (byte-identical). 22 tests (balanced-scan string/escape handling, greedy-regex fallback, all 4 sanitization attempts incl. a case that truly reaches the final bracket-strip fallback, 200-char-bounded SyntaxError). Also fixed the 2 stale "Opus" comments inside detect.ts. 100% stmt/line/func on the new module. 2 review CYCLES (amended to add the genuine final-fallback test per Reviewer A's correct note that the original "final attempt" test actually hit attempt-3).
- **#571 (living-artifact accuracy):** corrected ALL 7 remaining stale "Opus planner" comments outside detect.ts (`ios-plan/route.ts`, `types.ts`, 5× `DetectingStep.tsx`) → "Sonnet 4.6" (CLAUDE_PLANNER = claude-sonnet-4-6 since B4). Comment-only. After #570+#571, ZERO stale "Opus" refs remain in web/src (grep-confirmed). NOTE: Reviewer B caught my original "last two" commit-message overclaim (DetectingStep.tsx had 5 more) → I expanded the PR + corrected the message before merge. Lesson: grep the WHOLE tree before claiming "the last N".
- **#572 (Track G, coverage — planner-critical math):** extracted `normalizeScoresAcrossBatches` (z-score per source → min-max [0,1]; decides which frames survive into the planner) into `src/lib/detect-normalize.ts` (byte-identical). 10 tests (empty passthrough, single/all-identical→0.5, single-source [0,1], cross-source magnitude comparability, ordering+midpoint, clamp, <0.001 std-floor, field preservation, immutability). 100% coverage on new module.

**VERIFIED DROPS this run (do NOT re-propose without the named precondition):**
- **Security lens (Haiku scout, all 26 routes): CLEAN — no new Track-H gap.** Every paid/public/polling route has per-IP rate limit + body cap + per-field bounds + KV-atomic fail-closed entitlement/quota + spend ceilings + error hygiene + locked CORS/CSP/HSTS + Turnstile. Matches Run 89's clean finding. rate-limit.ts in-memory Map = intentional accepted residual.
- **COGS frame-score caching: RE-DROPPED (same as Run 89).** Hashing input frames to cache scored output is speculative pre-PMF infra: personal-video product → cross-user/cross-submission identical-frame reuse ≈ 0, requires an invented reuse %, and adds KV cost + wrong-scores correctness risk for unmeasured benefit. Do NOT re-propose absent real funnel data showing re-submission volume.
- **`email/index.ts` NOT a coverage gap** — it has `email.test.ts` (the test isn't named after the file; my initial `find` missed it). Don't "add email tests".


Cold start; synced to `origin/main` (tip **4d18fc0**, past Run 88's #565 housekeeping) FIRST; branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots + placeholder team/app IDs + credit-pack SKU = owner Mac), **functional_reality B** (free-tier watermark overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side UserDefaults = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit barred)). GROWTH_STATUS pre_launch, funnel/pmf 0/null, site DNS still not resolving (owner). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline GREEN (`npm ci` + `build` + `test` + `npm run lint` = 0 warnings, all exit 0). Ran a full **7-Haiku DEEP AUDIT** (last was Run 84, 2026-07-18, ~2 days) + VERIFIED every finding myself before deciding. Full 2-Sonnet-reviewer gate per change (6 reviewers, all APPROVE; both C-reviewers independently re-ran the mutation); disjoint 3-branch fan-out, all auto-merged on all required checks green.

**DEEP AUDIT — 2026-07-20 (Run 89) — 7 read-only Haiku lenses** (Track-H security, correctness/functional-reality, test-coverage, COGS/model/perf, artifact-freshness/honesty, design-taste/a11y, monetization/growth/business-case). 5 lenses CLEAN; the COGS, coverage, a11y, and monetization lenses each surfaced candidates — distilled to the 3 shipped PRs below + the verified drops. Every finding I verified against the actual source before acting.

**What shipped (3 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#566 (Track B, COGS observability):** `separateStems()` (reachable in prod via `/api/stems` → `isolateInstrumental`) makes a billed ElevenLabs `/audio-isolation` call on every music+voiceover export but was the ONLY paid ElevenLabs op with no `[CostMeter]` line (tts/sfx/music all log via `logProviderUsage`) — so per-export COGS reconciliation (BUSINESS_CASE §3, "verify cost from logs+invoices") silently under-counted this line. Added a success-path-only `logProviderUsage({provider:"elevenlabs", op:"stems", units: inputBytes/24000 seconds, meta:{inputBytes, instrumentalBytes}})` (best-effort, never throws; 24KB/s heuristic copied verbatim from the sibling music lib; no USD rate hardcoded). Tests assert the line fires on success and NOT on the failure path. Both reviewers APPROVE.
- **#567 (a11y, WCAG 2.4.7):** globals.css scopes its focus outline to `button:focus-visible` ONLY, so `<a>` links get NO visible keyboard-focus indicator unless they carry their own. The landing hero "Already have the app?" App-Store `<a>` and the ExportStep "Download Thumbnail" `<a>` were the LAST TWO `<a>` in web/src lacking one (grep-confirmed). Added the established `rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/60` — byte-for-byte the sibling nav/footer convention (#545/#564). className-only. Both reviewers APPROVE (Reviewer B git-grep-confirmed these were the last two uncovered `<a>`).
- **#568 (Track G, coverage — revenue-critical):** `/api/score` + `/api/ios-score` shape the response as `decision.isPro ? -1 : Math.max(0, decision.remaining - 1)`. The `Math.max` floor is exercised ONLY when `decision.remaining===0` — the viaCredit case (monthly quota exhausted, export authorized by a bought credit pack). Existing success tests only drive a fresh user (5→4), so a mutation dropping `Math.max` would return -1 (the Pro/unlimited sentinel) for a paying credit user → the free limit silently stops enforcing, and the suite stayed green. Added a route-level test to EACH route (independent copy-pasted calc instances): exhaust monthly → grantCredits → score → assert `remaining=0` (not -1) AND one purchased credit spent (getCreditBalance decremented, monthly counter untouched). Distinct IP (avoids the shared PAID_RATE_LIMIT bucket) + ceiling reset. MUTATION-VERIFIED by me AND independently by BOTH Sonnet reviewers (drop Math.max in both routes → exactly the 2 new tests fail, other 20 stay green). Test-only.

**VERIFIED DROPS this run (do NOT re-propose without the named precondition — saves future scout budget):**
- **Security lens (all 26 routes): CLEAN — no new Track-H gaps.** Every paid/public/polling route has per-IP rate limit + pre-parse body cap (or inline Content-Length) + per-field bounds + KV-atomic fail-closed entitlement/quota + spend ceilings + error hygiene + locked CORS/CSP/HSTS + Turnstile + proxy-video SSRF allowlist + timingSafeEqual on growth/stats. rate-limit.ts in-memory Map = intentional defense-in-depth (unchanged accepted residual).
- **Correctness/functional-reality lens: CLEAN.** All external/LLM/fetch calls have timeouts < serverless budget; all routes root try/catch; entitlement fails CLOSED, consumeExport fails OPEN (correct asymmetry); render/validateTape return 501 (no fake success); critical env fails loud.
- **Freshness lens: two findings were BOTH FALSE ALARMS (verified against code).** (1) "10 color filters" vs `ALL_FILTERS.length===11` — the 11th case is `"None"` (no-filter), so 10 NAMED color filters is CORRECT (a recurring known drop; the auditor missed None). (2) "bounce" marketed vs web `kinetic-text.ts` mapping Neon→flicker — the auditor only read the WEB preview; the shipped iOS product's `KineticCaptionRenderer.swift:28-29,117` genuinely implements `.bounce`→addBounceCaption ("Supports pop, bounce, slide, and typewriter"), so the marketing describes the iOS app correctly. Do NOT re-propose either.
- **COGS lens second finding (validation regen cap = 3 not 2): DROPPED.** validate/route.ts:186 comment "Maximum 3 regeneration requests per response to keep cost bounded" is the INTENTIONAL documented cap on USER-requested regenerations — distinct from plan.md's ≤2 VALIDATION-pass loop. Not a bug.
- **Monetization lens: the ONE candidate (server-side frame-score caching on /api/ios-score) was DROPPED as speculative pre-PMF infra.** For a PERSONAL-video product cross-user reuse ≈ 0; same-user re-encode often changes jpeg bytes → cache miss anyway; the "20% reuse" is invented (zero measured traffic); it adds a KV read/write to every scoring call AND a wrong-scores cross-contamination correctness risk (bad key → wrong highlights) for unmeasured benefit. Not clearly value-bar-clearing pre-launch. PRECONDITION to build: real post-launch reuse data showing a meaningful same-video re-score rate + a content-bound key design that can't cross-contaminate.
- **Design-taste lens: 18 "missing focus ring" findings were MOSTLY NOT gaps.** globals.css `button:focus-visible { outline: 2px solid var(--accent); outline-offset:2px }` gives EVERY `<button>` a WCAG-compliant focus indicator; the flagged editor-step BUTTONS already have it (adding a ring on top = stylistic churn, Reviewer-B-reject). The genuine gaps were only the two `<a>` links (uncovered by the button-only rule) → #567. Do NOT re-propose focus rings on `<button>` elements — the global outline covers them.

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (COGS metering completes no NEW DoD box; a11y + G coverage already broadly [x]; G5 DEEP AUDIT is standing/recurring); NO business-case recompute (metering adds observability, not a COGS-RATE change; no lever/pricing shift). Remaining DoD work stays owner/iOS/CI-gated (Mac archive A6 + 6.9" screenshots + team/app IDs + credit-pack SKU, owner eval key for tests_evals A, `.github` edits, live DNS). Three genuine value-bar-clearing wins (a real COGS-telemetry gap on the last unmetered paid op, a real WCAG keyboard-focus gap on the last two `<a>` CTAs, a mutation-effective revenue-critical route-contract lock) — the maximal disjoint set this DEEP AUDIT surfaced. The freshness lens's two false alarms + the design lens's 18-vs-2 over-report + the monetization frame-cache trap are the notable saves: each verified against real source before spending a branch.

## Run 88 — 2026-07-19 — 2 merged PRs (#563 G-coverage social connectedChannels mapping · #564 a11y UploadStep remove-button focus) — no deep audit (Run 84 ran DEEP AUDIT 2026-07-18, <24h/~4 runs)
Cold start; synced to `origin/main` (tip **3390510**, past Run 87's #560 housekeeping + #561 gtm + #562 planner-effort COGS) FIRST; branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots + placeholder team/app IDs + credit-pack SKU = owner Mac), **functional_reality B** (free-tier watermark overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side UserDefaults = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit barred)). GROWTH_STATUS pre_launch, funnel/pmf 0/null, site DNS still not resolving (owner). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline GREEN (`npm ci` + `build` + `test` = 1178 passed/84 files, 95.5% stmts + `npm run lint` = 0 warnings, all exit 0). Full **6-Haiku scout sweep** (Track-H security, test-coverage, artifact-freshness, COGS/correctness, monetization/growth, design-taste/a11y) + VERIFIED every finding myself before deciding. Full 2-Sonnet-reviewer gate per change (4 reviewers, all APPROVE); disjoint 2-branch fan-out, both auto-merged on all required checks green.

**What shipped (2 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#564 (a11y, design-taste):** the UploadStep per-thumbnail Remove button used `opacity-0 group-hover:opacity-100` with NO `focus-visible` variant — a keyboard user tabbing to it saw nothing (the delete action was reachable but invisible while focused; WCAG 2.4.7 focus-visible / keyboard-operable). Added `focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/60`, byte-for-byte the sibling reorder handle's pattern one element above (`UploadStep.tsx:425`) — established convention, not invented styling (continues #545/#529 a11y focus work). className-only; `--accent` confirmed real token (globals.css:9); build + lint green. Reviewer A verified pattern-consistency + eslint-clean + no regression; Reviewer B confirmed a genuine WCAG fix, not padding.
- **#563 (Track G, coverage):** `test(social)` pinning each `connectedChannels()` credential→channel arm (`social/queue.ts:49-51`). The suite only tested the `X_API_BEARER_TOKEN` branch; instagram/tiktok/reddit were uncovered. A mutation that cross-wired one (`INSTAGRAM_ACCESS_TOKEN → push("x")`) would silently route a queued post to the WRONG poster in `publish()` and passed the suite unchanged. New per-channel assertion pins each arm + confirms the others stay dry-run; MUTATION-VERIFIED by me (instagram→"x" fails, reverts green) AND independently re-verified by Reviewer A. Test-only. In the spirit of #559/#535.

**VERIFIED DROPS this run (do NOT re-propose without the named precondition — saves future scout budget):**
- **Security scout: the 5 media-route body-limit findings (animate/submit, talking-head, style-transfer, thumbnail, upscale) are REDUNDANT, NOT gaps.** The scout proposed adding `enforceBodyLimit()` to these routes claiming a Content-Length-omission DoS. But `enforceBodyLimit()` ONLY reads `Content-Length` (body-limit.ts source + its own doc comment: it "mirrors the inline `MAX_BODY_SIZE` guard the image/video/audio routes (voice-clone, thumbnail, upscale, animate/submit, style-transfer, talking-head, proxy-video) already apply") — and every one of those 5 routes ALREADY has the identical inline `if (contentLength && parseInt(contentLength) > MAX) return tooLargeResponse()` check before `req.json()`. So adding `enforceBodyLimit` provides ZERO protection the inline check doesn't = pure churn (Reviewer-B-reject). The chunked/no-Content-Length case is an ACCEPTED, documented codebase-wide tradeoff (body-limit.ts NOTE: "the AUTHORITATIVE bound remains the per-field size caps applied AFTER parse" — which these routes have via `overStringLimit`). Matches Run-85's "upscale/thumbnail/talking-head/style-transfer inline-capped" note. Do NOT re-propose enforceBodyLimit on these 5 routes — it duplicates an existing inline guard.
- **Monetization scout: all 3 growth candidates (wire landing-headline A/B; register pricing-cta/upgrade-clarity experiments; rewrite pricing "upgrade clarity" copy) are PREMATURE.** The `EXPERIMENTS` registry comment in `experiments.ts` EXPLICITLY stages this: for landing-headline it says "Wire the render + exposure call when the site goes live with traffic (pre-launch there is nothing to measure)." Wiring the exposure/`recordEvent` call now contradicts the code's OWN intentional staging decision + pre-PMF discipline (site DNS doesn't even resolve; funnel/pmf 0/null; no traffic to measure) + would require a new client→server exposure round-trip for zero pre-launch value. Registering MORE experiments compounds the same premature wiring. The pricing-copy rewrite is stable-anchor churn (current "Start free. Go Pro when you're ready to scale" is fine; the 5-export limit is already stated in the Free card). **PRECONDITION to build: when the owner brings the site live with real traffic (DNS resolves + Plausible connected), wire the landing-headline exposure/conversion call to the hero — the E8 engine is ready.**
- **Test-coverage scout: `#563` was the one genuine gap.** email/index.ts:114 `unsupported_provider` fallback = unreachable (emailProvider() only returns "resend"|"none"); beat-sync.ts:272 `quality=1` when totalTransitions===0 = guarded-unreachable; experiments.ts:88 float-edge fallback = vanishing-probability defensive. All dropped as impossible-case/dead-code.
- **COGS/correctness scout: CLEAN.** All external/fetch/LLM calls have timeouts <60s serverless budget; all routes root try/catch; KV quota/ceiling/credit atomic; entitlement fails closed, consume fails open (correct asymmetry). The `/api/score` route NOT using ephemeral prompt caching (vs `/api/ios-score`) is an INTENTIONAL tradeoff — its system prompt embeds the per-request frame-timestamp list so a cache hit is unlikely, and the call is gated by the 5/mo free quota anyway. Not a bug. Do NOT propose "add caching to /api/score" without a design that keeps the timestamps out of the cached prefix.
- **Artifact-freshness scout: CLEAN — zero contradictions.** Pricing uniform ($14.99/$149.99), free 5/mo + watermark, 50/day ceiling, 1080×1920 export, 4 animated caption styles (pop/bounce/slide/typewriter — flicker/fade static, not marketed), 11 color-filter cases vs "10 named", disabled-v1 features honestly marked, no-persistent-frame-storage + no-embedded-keys claims match code, 480px/512px frame sizes match. Re-confirms Runs 80-87.
- **Design-taste scout: `#564` was the one genuine a11y gap.** Landing / ResultsStep / EditorStep / ExportStep / header / footer / Turnstile all clean (real lucide icons, alt text, semantic HTML, focus rings, aria attributes, 44/48px targets, no slop words, no emoji-as-UI). Only the UploadStep remove-button focus-visibility miss.

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (G coverage + a11y already broadly [x]; these complete no NEW DoD/Track box); NO business-case recompute (no lever/pricing/COGS change). Remaining DoD work stays owner/iOS/CI-gated (Mac archive A6 + 6.9" screenshots + team/app IDs + credit-pack SKU, owner eval key for tests_evals A, `.github` edits, live DNS). Two small genuine value-bar-clearing wins (a real keyboard-a11y gap on a core surface + a real cross-wiring coverage lock), maximal disjoint set this sweep surfaced — a coherent maintenance run. The security scout's 5 "findings" being redundant-with-existing-inline-guards is the notable save this run: verified against the actual `enforceBodyLimit` source before spending a branch on churn.

## Run 87 — 2026-07-19 — 2 merged PRs (#558 E7 waitlist funnel analytics · #559 G-coverage beat-sync fallback test) — no deep audit (Run 86 recorded DEEP AUDIT 2026-07-18 via #547, <24h)
Cold start; synced to `origin/main` (tip **ac999ed**, past Run 86's #557 housekeeping) FIRST; branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots + placeholder team/app IDs + credit-pack SKU = owner Mac), **functional_reality B** (free-tier watermark overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side UserDefaults = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit barred)). GROWTH_STATUS pre_launch, funnel/pmf 0/null, site DNS still not resolving (owner). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline GREEN (`npm ci` + `build` + `test` + `npm run lint` = 0 warnings, all exit 0). Full **5-Haiku scout sweep** (Track-H security, test-coverage, functional-reality/export-count, artifact-freshness, monetization/growth) + VERIFIED every finding myself. Full 2-Sonnet-reviewer gate per change (4 reviewers, all APPROVE); disjoint 2-branch fan-out, both auto-merged on all required checks green.

**What shipped (2 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#558 (Track E7, growth analytics):** the landing `WaitlistForm` fired only `waitlist_signup` — the failure path and the pending double-opt-in step were invisible. Added `waitlist_error { reason: "network"|"server" }` (coarse category ONLY — never the raw `data.error`; no PII, bounded cardinality, type-enforced via a literal union — both reviewers independently asked for exactly that tightening, applied) and `waitlist_confirmation_sent` (accepted-but-pending → exposes signup→confirm drop-off AND email-deliverability failures = signups with no confirmations). Instrumentation lands BEFORE Plausible connects so day-1 launch data has no gap; `trackEvent` stays a no-op until `window.plausible` exists (zero pre-launch runtime cost). analytics.ts taxonomy + analytics.test.ts + landing/page.tsx. NOTE: no jsdom/testing-library in the repo, so component-level firing is not unit-testable — the 6 pre-existing events follow the SAME pattern (wired in page.tsx, taxonomy tested in analytics.test.ts); web-e2e covers render.
- **#559 (Track G, coverage):** `test(beat-sync)` locking `nextBeatAfter`'s past-the-last-beat fallback (`beat-sync.ts:55` `return time`) — untested tail branch. `nextBeatAfter` has NO production caller yet (components import buildBeatGrid/getBeatIntensity/validateTimeline), so this pins the exported utility's documented contract (returns the time, never undefined/NaN) before a caller wires it; the `number` return type catches only a signature change, not a logic regression (e.g. a bad `grid.beats.at(-1) ?? NaN`). Reviewer A MUTATION-TESTED the source (`return undefined` → the new test fails) to prove it's not a vacuous pass; Reviewer B's no-caller caveat folded into an honest commit message (dropped the overstated "silently misplace clips" active-path framing).

**VERIFIED DROPS this run (do NOT re-propose without the named precondition — saves future scout budget):**
- **Security scout: CAPTCHA-fail-closed on `/api/waitlist` = a TRAP, NOT a gap.** The scout proposed 503-ing the form when `TURNSTILE_SECRET_KEY` is unset. But the fail-OPEN is INTENTIONAL + documented: PENDING_OPS.md:318 explicitly keeps `TURNSTILE_SECRET_KEY` UNSET in CI so CAPTCHA fails open (so web-e2e's self-seeding journeys pass), and REMAINING_STEPS 2b documents the full owner Turnstile setup. Failing closed would 503 the ENTIRE pre-launch waitlist (domain isn't even live) AND break web-e2e — gating a live flow on an unconfigured dependency (DECISION COROLLARY). Rate-limiting (PUBLIC_RATE_LIMIT 5/min/IP) + the Turnstile-fails-open path with a 5s timeout is the correct graceful degradation. Do NOT re-propose CAPTCHA-fail-closed without the owner having set the Turnstile keys.
- **Functional-reality scout: a new `/api/export/consume` endpoint = SPECULATIVE.** The export-COUNT quota being client-side (iOS UserDefaults) is the real ship-critical gap, but building a server endpoint iOS DOESN'T CALL doesn't move it — the actual product still enforces client-side until the iOS local-export path is wired to call it (iOS work, can't verify on Linux). A dangling public endpoint adds attack surface + is dead code. PRECONDITION to build: wire it TOGETHER with the iOS `ExportView` pre-export call in the same body of work (owner/iOS-gated), not ahead of it.
- **Test-coverage scout: sfx-library.ts matching path = DEAD CODE.** The CDN-library keyword-scoring/threshold path (sfx-library.ts:87-104) never executes because every library entry currently has `url:null`; testing the matching of an empty library is impossible-case coverage until the CDN library is populated (owner asset work). The chosen beat-sync fallback (a REACHABLE exported-utility contract) was the cleaner win. Other coverage candidates (kinetic-text boundary, store.ts reducer) were vague ("likely involve") — skipped as un-actionable.
- **Monetization scout: PWA/App-Store screenshots in manifest.json + funnel copy = can't do.** Adding real PWA/store screenshots needs actual rendered images I can't fabricate (owner/design). Two of the four monetization candidates (error-state + confirmation-sent tracking) WERE the genuine core of #558.
- **Artifact-freshness scout: CLEAN — zero contradictions.** Pricing uniform ($14.99/$149.99), free 5/mo + watermark, 50/day ceiling, disabled-v1 features honestly marked, no-persistent-frame-storage privacy claim matches /api/score, "no embedded keys" matches CloudScoringService. Re-confirms Runs 80-86.

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (E7 analytics engine + G coverage already broadly [x]; these complete no NEW DoD/Track box); NO business-case recompute (no lever/pricing/COGS change). Remaining DoD work stays owner/iOS/CI-gated (Mac archive A6 + 6.9" screenshots + team/app IDs + credit-pack SKU, owner eval key for tests_evals A, `.github` edits, live DNS). Two small genuine value-bar-clearing wins (a real funnel-visibility gap + a real coverage lock), maximal disjoint set this sweep surfaced — a coherent maintenance run.

## Run 86 — 2026-07-19 — QUIET RUN, 0 code PRs (nothing cleared the value bar) — no deep audit (Run 84 ran one 2026-07-18, <24h/~2 runs)
Cold start; synced to `origin/main` (tip **4e8cbbf**, past Run 85's #555 housekeeping + #556 Impeccable-design-skill vendor + FACTORY_STANDARD §6b/§44) FIRST. NOTE: local `main` ref was stale at 5eb8141 on entry (looked like "3 commits ahead"); `origin/main` == my branch == 4e8cbbf, so the state is fully-merged/clean — #554/#555/#556 all landed. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots + placeholder team/app IDs + credit-pack SKU = owner Mac, 8th+ cycle), **functional_reality B** (free-tier watermark overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side UserDefaults = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit barred)). GROWTH_STATUS pre_launch, funnel/pmf 0/null, site DNS still not resolving (owner). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline GREEN (`npm ci` + `build` + `test` [MARGIN_* set this session but suite passed] + `npm run lint` = 0 warnings, all exit 0). Full **5-Haiku scout sweep** (Track-H security, correctness/COGS, test-coverage, artifact-freshness/marketing, monetization/growth-engine) + VERIFIED every finding myself before deciding. **Every candidate was a false alarm, non-mutation-effective, already-covered, premature/owner-coupled, or stable-anchor churn — shipping any would be padding. Quiet coherent run = success (per mandate). No code branch/PR opened beyond this housekeeping record.**

**VERIFIED DROPS this run (do NOT re-propose without the named precondition — saves future scout budget):**
- **Security scout: ZERO genuine Track-H gaps** — all 13 paid routes + public forms have per-IP rate limit + pre-parse body cap + per-field size bounds + error-hygiene + KV-atomic cross-instance daily ceilings + server-side JWS entitlement + locked CORS/CSP/HSTS + proxy-video SSRF allowlist. Re-confirmed the rate-limit.ts in-memory Map is DEFENSE-IN-DEPTH ONLY (the authoritative wallet guard is the KV-atomic per-user 500-gen/day + global validate ceilings, cross-instance) — NOT actionable; matches the scorecard's own "(To A+)" residual note. Do NOT re-propose "KV-back rate-limit.ts" — it adds a KV op/latency to every request for a bound H7 already enforces cross-instance.
- **Correctness scout: `consumeExport()` credit-path returns `FREE_EXPORT_LIMIT` not the live count (entitlement.ts:~260)** — DROPPED. The return value is UNUSED by every caller (score/ios-score never read it) AND `FREE_EXPORT_LIMIT` is semantically correct on the credit path (monthly quota exhausted). Fixing a return no one reads is marginal churn. Do NOT re-propose without a caller that actually consumes the return.
- **Coverage: `verifyProEntitlement` >MAX_SIGNED_TRANSACTION_CHARS length guard (entitlement.ts:94)** — DROPPED as NOT mutation-effective. Removing the guard doesn't flip the boolean: a multi-MB garbage JWS fails `verifyAppStoreJWS` → `false` anyway; the guard only changes an internal DoS/perf property a test can't cleanly observe. SAME class as Run-82's dropped app-store-jws:101 x5c guard. Do NOT re-add.
- **Coverage: `redeemCreditPack` `txn.productId ? CREDIT_PACK_PRODUCTS[…] : undefined` missing-productId branch (entitlement.ts:311)** — DROPPED as NOT mutation-effective. Removing the ternary → `CREDIT_PACK_PRODUCTS[undefined]` → `undefined` → `if(!amount)` rejects with the SAME reason. Same result either way; no clean mutation-effective test. Do NOT re-add.
- **Coverage: over-long `userId` → 429 at ROUTE level (generation-ceiling routes)** — DROPPED as ALREADY-COVERED-TWICE + mostly-dead-code. `isValidUserId`'s length bound is unit-tested at `user-id.test.ts:13-15` (MAX+1 AND 1,000,000 chars) AND the ceiling function's fail-closed-on-over-long path at `spend-ceiling.test.ts:85` ("fails closed on an over-long userId and records nothing (H2)", `"u".repeat(500)`). A route-level HTTP permutation composes three already-covered pieces → locks NO new invariant. Also: 9 of the 13 `enforceGenerationCeiling` routes (intro/outro/sfx/style-transfer/talking-head/thumbnail/upscale/voice-clone/voiceover) are v2-DISABLED = dead-code coverage. Do NOT re-propose.
- **Coverage: `Number(balanceRaw) || 0` KV-Lua garbage floor (credit-store.ts:160)** — DROPPED as impossible-case defensive coverage (the in-repo Lua returns a numeric INCRBY result; it never yields NaN/"garbage"). Reviewer-B-reject class.
- **Growth/monetization scout: 5 analytics-instrumentation candidates (credit-pack aggregates in E7 `getGrowthMetrics`; `recordEvent` on credit-redeem, free-limit-hit, credit-spend; typed experiment conversions)** — ALL DROPPED as PREMATURE + owner-coupled + critical-path-risky. (1) The "→ trial → paid" write SOURCE is structurally owner-blocked pre-launch: credit purchases need the iOS StoreKit consumable UI + `.storekit` SKU (both owner-gated), and Pro needs live billing — so any credit/revenue aggregate is always 0 until the owner ships them; building the READ ahead of the WRITE is hollow infra, and the natural coupling is to add the purchase event + its aggregate TOGETHER when the owner builds iOS purchase. (2) #2-#4 inject `recordEvent` KV writes into `entitlement.ts` `checkExportAllowed`/`consumeExport` — the critical wallet gate — adding latency/COGS/a failure surface to EVERY paid export check, for events that can't fire pre-launch (no users, no live KV, GROWTH_STATUS datastore unavailable). (3) Pre-PMF discipline: prioritize PRODUCT, do NOT build revenue analytics ahead of any traffic ("no growth into a leaky bucket"). E8-precedent (engine ships before live) does NOT extend here because E8 was one self-contained engine; E7's credit read needs the owner-blocked write to have value. **PRECONDITION to build: when the owner ships the iOS credit-purchase UI + `.storekit` SKU (store_readiness fix), add the server-side purchase/free-limit-hit events + their E7 aggregates in the SAME work as the purchase path — off the hot entitlement gate where practical.**
- **Freshness scout: README "web is the waitlist/marketing FUNNEL, not a usable web app" (README:10-11) vs the gated web editor** — DROPPED. The web editor exists but is SITE_GATE_PASSWORD-gated pre-launch; the README states the STRATEGIC posture (native-first, web=funnel) which ALIGNS with GTM_STANDARD §12. Rewriting it to declare web a co-equal product is a STRATEGY change on a scout's inference that would risk CONTRADICTING §12 — not a clean freshness fix. Do NOT churn this stable anchor without an actual owner strategy decision.
- **Freshness scout: landing "Share directly to TikTok, Instagram Reels, YouTube Shorts" (landing/page.tsx:191)** — DROPPED. This exact copy has survived 80+ freshness sweeps (all "ZERO drift") that specifically check marketing-vs-unbuilt (I-track direct-social-publish) claims; the iOS share sheet genuinely one-taps a video into TikTok/IG/YouTube, so "share directly" is a defensible description of the share-sheet UX (the FAQ also clarifies). Re-litigating it is churn. Do NOT re-propose without an owner honesty directive.
- **Freshness scout: template divergence (9 web incl. Wedding / 8 iOS)** — DROPPED as not customer-facing (already documented in templates.ts comments per #476; no false claim to users).

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (no DoD/Track item advanced); NO business-case recompute (no lever/pricing/COGS change). Remaining DoD work stays owner/iOS/CI-gated (Mac archive A6 + 6.9" screenshots + team/app IDs + credit-pack SKU, owner eval key for tests_evals A, `.github` edits, live DNS). The loop is in its converged maintenance state — each run's honest output is a small win or (this run) a clean no-ship with recorded drops. A quiet coherent run.

## Run 85 — 2026-07-18 — 1 merged PR (#554 H2 body-cap on `/api/animate/check`, the last unguarded JSON route) — no deep audit (Run 84 ran one 2026-07-18, <24h)
Cold start; synced to `origin/main` (tip 5eb8141, past Run 84's #547 housekeeping + the canonical FACTORY_STANDARD §51–55 doc syncs #549–553) FIRST; branched the change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac), **functional_reality B** (overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit barred)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline GREEN (npm ci + build + `env -u MARGIN_* vitest` = 1173 passed / 84 files + `npm run lint` = 0 warnings, all exit 0). Full **4-Haiku scout sweep** (test-coverage, Track-H security, correctness/functional-reality, artifact-freshness) + VERIFIED every finding myself. 3 scouts clean; the security scout surfaced the one real gap. Full 2-Sonnet-reviewer gate (both APPROVE).

**What shipped (1 PR, 2-reviewer + all required checks green, auto-merged):**
- **#554 (Track H2, security):** `enforceBodyLimit(req, JSON_BODY_LIMIT_BYTES)` (1 MB) on `/api/animate/check` — the LAST JSON-parsing route still calling `req.json()` with no pre-parse Content-Length cap. It is a deployed, unauthenticated animation-status poll with NO feature gate (contrast `/api/render`, which 501s BEFORE `req.json()` → zero prod exposure, correctly left unguarded), so an over-declared body was buffered into memory before the tiny `predictionId` was read; `POLL_RATE_LIMIT` bounds request COUNT not body SIZE. Same pre-parse memory-DoS surface #544 closed on credits/redeem + waitlist + growth/experiment. Guard after the rate-limit check, before `req.json()`; a real poll body is a few dozen bytes so 1 MB never rejects legit traffic. Mutation-effective 413-before-parse test added to `body-limit-wiring.test.ts` (drop guard → 500, not 413; verified by removal/rerun). Reviewer A caught an overstated "last route" code comment (`/api/render` also lacks it, but is dead-code-gated) → I reworded it to name the actual siblings instead of claiming "last"; Reviewer B re-grepped all 22 `req.json()` routes + confirmed the render/check asymmetry from source.

**DROPS this run (3 scouts clean — do NOT re-propose without the named precondition):**
- **Test-coverage scout: ZERO new mutation-effective REACHABLE gaps** — spend-ceiling/entitlement/app-store-jws/rate-limit/credit-store/kv-quota/input-bounds/body-limit/growth-metrics all mutation-complete for v1-reachable code (re-confirmed Runs 82–84).
- **Correctness scout: ZERO genuine bugs** — all external/fetch calls have timeouts < 60s serverless budget; all routes have root try/catch; KV quota/ceiling/credit math atomic (INCR/DECR/Lua); entitlement fails closed, consume fails open (correct asymmetry). Re-confirms Runs 82–84.
- **Artifact-freshness scout: ZERO drift** — prices ($14.99/$149.99), free 5/mo + watermark, 50/day ceiling, disabled-v1 features (audio/photo-anim/intro-outro), COGS/margin, privacy frame dims (480p web / 512 iOS), 1080×1920 export, BUSINESS_CASE_SUMMARY-vs-body all reconcile. Re-confirms Runs 80–84. The "50/day fair-use" landing parenthetical is a DELIBERATE honesty disclosure — do NOT strip to "Unlimited".
- **Security scout: full Track-H stack complete on all OTHER live routes** — the only gap was #554; the 7 other req.json() routes without `enforceBodyLimit` are v2-disabled (animate/submit uses inline 20MB, upscale/thumbnail/talking-head/style-transfer inline-capped) or `/api/render` (501 before parse) or the test file.

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (H2/G1 already [x]; the body-cap extension completes no NEW DoD/Track box); NO business-case recompute (no lever/pricing/COGS change). Remaining DoD work stays owner/iOS/CI-gated (Mac archive A6, owner eval key, `.github` edits, live DNS). A quiet, coherent maintenance run — one real gap closed that the #544 sweep missed, exactly the class of prior-sweep-miss Run 84 documented.

## Run 84 — 2026-07-18 — 3 merged PRs (#544 H2 body-cap on 3 missed JSON routes · #545 a11y focus rings on 2 missed `<a>` CTAs · #546 spend-ceiling/jws security-gate coverage) + DEEP AUDIT 2026-07-18
Cold start; synced to `origin/main` (tip ad0496f) FIRST, branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac, 8th cycle), **functional_reality B** (free-tier watermark overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side UserDefaults = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit I'm barred from)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling). BUSINESS_CASE honest, base y1 $7,740, floor not met y1 (~month-38) — all consumed as DATA. Web baseline GREEN (npm ci + build + `env -u MARGIN_* vitest` = 1167 passed/84 files + `npm run lint` = 0 warnings). Full 2-Sonnet-reviewer gate per change (6 reviewers, all APPROVE); multi-branch disjoint fan-out worked.

**DEEP AUDIT — 2026-07-18 (Run 84) — 6 read-only Haiku lenses** (security/abuse, correctness/functional-reality, test-coverage, COGS/model, artifact-freshness/honesty, design-taste/a11y). ~1 day since Run 80's audit. VERIFIED every finding myself before acting → distilled to the 3 shipped PRs below + the verified drops. **This audit was PRODUCTIVE (unlike Runs 79-83) — the security + a11y lenses each surfaced a real gap a prior sweep missed.**

**What shipped (3 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#544 (Track H2, security — the run's anchor):** the #530 body-cap sweep hardened the paid vision/planner routes but MISSED three JSON routes that still called `req.json()` with no pre-parse Content-Length guard: `/api/credits/redeem` (PAID StoreKit credit-pack redeem — JWS verify + KV writes), `/api/waitlist` (public double-opt-in form), `/api/growth/experiment` (public A/B beacon). Added `enforceBodyLimit(req, JSON_BODY_LIMIT_BYTES)` (1 MB, ~40x headroom) after the rate-limit check + before `req.json()` on each; extended `body-limit-wiring.test.ts` with a mutation-effective 413-before-parse assertion per route. Real live-endpoint hardening on the exact H2 pattern.
- **#545 (Track A5/D, a11y):** the #529 focus-ring sweep did the pricing CTA + legal/support links but MISSED two link-styled `<a>` CTAs — the landing header primary CTA (its Features/Pricing/FAQ siblings already had the ring) + the ExportStep "Go Pro on iOS" card. `<a>` gets no ring from the global `button:focus-visible` rule. Reused the shipped token.
- **#546 (Track G, coverage):** two mutation-effective security-gate branches — spend-ceiling VercelKV `get` success path (`?? 0`, prior tests only rejected get / exercised incr) + app-store-jws `loadTrustedRootsFromEnv` no-regex-match guard (`if (!blocks) return []`, distinct from the malformed-block test; no outer try/catch → removing it throws). Reviewer A confirmed via live mutation.

**DROPPED — verified false alarms / eval-gated / marginal (do NOT re-propose without the named precondition):**
- **COGS: "rebuild `clipFrames` after `applyClipFixes` removes clips (DetectingStep.tsx:~1425-1531) to avoid stale indices on validate pass 1"** — DEFERRED. It CHANGES what the validator sees (frame-to-clip caption alignment) on the core-detection path → needs G3 real-AI eval evidence, keyless-blocked from Linux; lives in a client React component I can't drive headlessly; the scout's claim was uncertain ("wrong associations OR out-of-bounds"); saving is tiny (~$0.004/validation, only when clips are removed). **PRECONDITION to pursue: a runnable G3 validation-quality eval proving the re-indexed input doesn't shift validation/regeneration outcomes.**
- **Correctness: `ios-validate/route.ts:245` + `validate/route.ts:415` `typedClips[frame.clipIndex]` out-of-bounds** — DROPPED. The `clip?.captionText || "(no caption)"` optional chaining is INTENTIONAL graceful degradation — an out-of-bounds index yields a slightly mislabeled caption in the validator's vision prompt (frames already body-limited before this), NOT a crash / data-corruption / wallet-drain / bypass. Adding a bounds guard is marginal defensive churn on a non-critical label. Do NOT re-propose.
- **Coverage: `rate-limit.ts:70` E2E_RATELIMIT_BYPASS runtime early-return** — DROPPED as marginal (testing a test-only escape hatch; the real security control — the boot-guard `assertRateLimitBypassNotOnLivePlatform` — is already tested at rate-limit.test.ts:158). Low value.
- **Artifact-freshness scout: ZERO drift** (pricing $14.99/$149.99, free 5/mo+watermark, 50/day ceiling, disabled-v1 features, COGS/margin, BUSINESS_CASE_SUMMARY-vs-body all reconcile — consistent with Runs 80-83).
- **COGS scout: no safe non-eval win** (Haiku <4096-tok cache floor re-confirmed no-op; planner already caches; validation capped at 2 passes; frames dynamically bounded + 480p downscaled).

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (H2/G1 already [x]; the body-cap extension, a11y mop-up, and coverage complete no NEW DoD/Track box — G2 stays partial); NO business-case recompute (no lever/pricing/COGS change). The remaining DoD work is owner/iOS/CI-gated (Mac archive A6, owner eval key, `.github` edits, live DNS). A productive maintenance run — 3 real gaps closed that prior sweeps missed.

## Run 83 — 2026-07-18 — QUIET RUN, 0 code PRs (nothing cleared the value bar) — no deep audit (Run 80 ran one 2026-07-17, <24h/~3 runs)
Cold start; synced to `origin/main` (tip d4179e9) FIRST. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac), **functional_reality B** (overlay-pass export test hangs on CI simulator = iOS; export-COUNT quota client-side = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit I'm barred from)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling; site DNS still not resolving — owner). BUSINESS_CASE honest, base y1 $7,740 — all DATA. Web baseline GREEN (npm ci + build + `env -u MARGIN_* npm test` = 1167 passed / 84 files + `npm run lint` = 0 warnings, all exit 0). Full **6-Haiku scout sweep** (COGS/model, Track-H security, free→paid conversion surfaces, correctness/functional-reality, test-coverage, artifact-freshness) + VERIFIED every finding myself before deciding. **Every candidate a false alarm, a no-op, eval-gated, or an honesty regression — shipping any would be churn. Quiet coherent run = success (per mandate). No branch/PR opened beyond this housekeeping record.**

**VERIFIED DROPS this run (do NOT re-propose without the named precondition — saves future scout budget):**
- **COGS: "add `cache_control` to the `/api/validate` (frames path) system prompt"** — RE-CONFIRMED NO-OP (same class as abandoned #533). Measured `fullSystemPrompt` = `systemPrompt + outputFormat` ≈ **13.2k chars ≈ 3.3k tokens** (identical in the frames and no-frames paths — `hasFrames` only changes the USER content, not `system:`), which is **BELOW the Haiku-4.5 4096-token prompt-cache floor**, so `cache_control` is silently ignored (no cache read, no COGS benefit). The scout's "~189 lines exceeds 4096" was an over-estimate. Do NOT re-propose caching the validator/scorer system prompt on Haiku 4.5 (both <4096 tok). (Sonnet-4.6 planner is above its 1024 floor and already caches.)
- **COGS: "remove the timecode list / cap batch size / trim planner input on `/api/score`"** — all change what the SCORER/PLANNER actually SEE → core-detection-path changes requiring real-AI G3 eval evidence, which is keyless-blocked from Linux. DEFERRED (unchanged discipline). The timecode-text removal (score/route.ts:104, ~500-800 tok redundancy) is a genuine future COGS candidate IF eval evidence ever becomes runnable.
- **Security (Track H): "harden `/api/render` (rate-limit + body-limit + entitlement + dimension/array bounds)"** — DEFERRED (below the value bar as speculative dead-code hardening). Verified: the `RENDER_ENABLED !== "true"` feature gate returns **501 BEFORE `req.json()`**, so with the flag unset (the prod default) there is **ZERO production exposure** — no body is parsed, no paid API call exists anywhere in the route (even with the flag ON it validates the EDL then returns a SECOND 501 "render worker not yet deployed"). The web `ExportStep.tsx:145` calls it as a fast-path, caches the 501 (`serverRenderAvailable=false`, never re-probes), and falls back to client-side Canvas — non-broken. Guards would only matter once the owner both builds the FFmpeg worker AND flips `RENDER_ENABLED=true`, at which point they belong WITH that build (A1/B1). **PRECONDITION to build: when the server-side render worker actually lands.** Not a live wallet-drain surface today.
- **Conversion: "change landing pricing card `No monthly export cap (50/day fair-use)` → `Unlimited monthly exports`"** (landing/page.tsx:235) — REJECTED as an HONESTY REGRESSION. The 50/day ceiling applies to ALL tiers (an anti-abuse backstop, per BUSINESS_CASE §3/§4 + Terms/Support); the current parenthetical is a deliberate, reconciled honesty disclosure (freshness scout confirmed it reconciles across all surfaces). Flatly claiming "Unlimited" without the caveat would overclaim. Do NOT strip the fair-use disclosure to boost "conversion confidence."
- **Correctness scout: ZERO genuine bugs** (timeouts under the serverless budget, atomic KV quota/credit math, fail-closed entitlement all re-verified).
- **Test-coverage scout: ZERO mutation-effective REACHABLE gaps** — the critical entitlement/quota/credit/spend-ceiling/JWS/rate-limit/body-limit branches are mutation-complete for v1-reachable code. (Coverage this run: the suite is comprehensive; prior runs absorbed the standard gaps.)
- **Artifact-freshness scout: ZERO drift** — pricing ($14.99/$149.99), free tier (5/mo + watermark), 50/day ceiling, disabled-v1 features (audio/talking-head/style-transfer), COGS/margin, and the BUSINESS_CASE_SUMMARY-vs-body all reconcile.

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (no DoD/Track item advanced); NO business-case recompute (no lever/pricing/COGS change). The remaining DoD work is owner/iOS/CI-gated (Mac archive A6, owner eval key, `.github` edits, live DNS) — the loop has converged to a maintenance state where each run's honest output is a small win or (this run) a clean no-ship with recorded drops. A quiet coherent run.

## Run 82 — 2026-07-17 — 3 merged PRs (#539 JWS self-signed-root guard coverage · #540 scoring-timeout test hermetic to Margin · #541 growth-metrics KV-connected snapshot coverage) — no deep audit (Run 80 ran one 2026-07-17, <24h)
Cold start; synced to `origin/main` (tip b0a738f) FIRST, branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac), **functional_reality B** (overlay-pass export test hangs on CI simulator; export-COUNT quota client-side = iOS), **tests_evals B** (real paid-AI round-trip / `ios`→REQUIRED = owner key + `.github` edit I'm barred from)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling). BUSINESS_CASE honest, base y1 $7,740 — all DATA. Web baseline green (1165 tests pre-change). Full 5-Haiku scout sweep + VERIFIED every finding myself before acting. Full 2-Sonnet-reviewer gate per change; multi-branch disjoint fan-out worked. **maker≠checker EARNED ITS KEEP: Reviewer B killed a non-mutation-effective padding test inside the #539 anchor (see below).**

**ENV QUIRK — local `npm test` diverges from CI (record so future runs unset it):** this sandbox has `MARGIN_INGEST_KEY`/`MARGIN_INGEST_URL`/`MARGIN_*` SET, so `getMeter()` (margin-meter-client.ts) returns a REAL meter that fires an ingest `fetch` on `recordCall`. In keyless CI those are absent → `getMeter()` is null → no-op. This makes `detect-scoring-timeout.test.ts` (asserts scorer fetch called exactly once) FAIL locally (2 calls) while PASSING in CI. **Always run local verification with `env -u MARGIN_INGEST_KEY -u MARGIN_INGEST_URL -u MARGIN_OSS_INGEST_KEY -u MARGIN_DATABASE_URL -u MARGIN_DIRECT_DATABASE_URL -u MARGIN_ANTHROPIC_API_KEY npx vitest run` to match CI.** #540 fixed the one test this actually broke (only that test drives a SUCCESSFUL scoring path that reaches the meter emit; the validate-guard tests assert fetch-never-reached BEFORE the emit, so unaffected).

**What shipped (3 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#539 (Track G, security-critical coverage — anchor):** the self-signed-root guard of `verifyAppStoreJWS` (`app-store-jws.ts:119` `!root.verify(root.publicKey)`), the server-authoritative Pro-entitlement gate. Test builds a [leaf, intermediate] chain + trusts the intermediate DER directly so EVERY earlier guard passes (alg/x5c-len/validity/root-byte-match/leaf-verifies-under-intermediate) → the self-signed check is the ONLY thing that can null it (mutation-effective: deleting the line flips to a decoded payload). Blocks smuggling a mid-chain CA in as a "root" to forge entitlements. Branch cov 87.17%→~92%.
- **#540 (Track G, test robustness):** made `detect-scoring-timeout.test.ts` hermetic — assert on the SCORING call by its `api.anthropic.com` URL instead of a total-fetch-count, so the Margin telemetry ingest fetch (env-gated) no longer makes it spuriously fail locally. Passes identically with/without the Margin env. Test-only.
- **#541 (Track G, coverage):** the `storeLive` branch of `getGrowthMetrics` (source "kv" + "waitlist-store" channel + durable counts) — only fires with real KV env, never exercised by the in-memory suites. Mocked `@vercel/kv` (scard + hgetall; experiments.ts shares the KV gate) + stubbed KV env; differentiated mock values (7 vs 3) catch a key-mixup. Feeds `/api/growth/stats` connection-state fields. Branch 75%→100%.

**maker≠checker SAVE (#539):** the anchor originally added a SECOND test for the non-string-x5c-entry guard (`app-store-jws.ts:101`). Reviewer B REQUEST_CHANGES, correctly: `Buffer.from(42,"base64")` THROWS a TypeError that the function's outer `try/catch` already nulls, so deleting the `typeof entry!=="string"` guard does NOT change the outcome — the test was coverage padding indistinguishable from the existing malformed-input catch-all, not mutation-effective. Dropped it; kept only the mutation-effective self-signed-root test. (My selection error: verified "uncovered branch" but not "mutation-effective" before writing.) **DO-NOT-REPROPOSE:** the `app-store-jws.ts:101` non-string-x5c guard is redundant with the outer try/catch for OBSERVABLE behavior — no clean mutation-effective test exists for it; don't re-add a "cover line 101" test.

**DROPPED — verified false alarms / documented-design (do NOT re-propose without the named precondition):**
- **Correctness scout: 2 findings BOTH FALSE ALARMS.** Claimed `/api/validate` (route.ts:702) + `/api/proxy-video` (route.ts:90) `reader.read()` streaming loops can hang past the 60s budget because "AbortSignal.timeout only bounds the headers phase." FALSE — `AbortSignal.timeout` binds the WHOLE fetch incl. body streaming (validate has `signal:AbortSignal.timeout(45_000)` at :295; proxy-video `AbortSignal.timeout(30_000)` at :64), so a stalled read aborts at 45s/30s < 60s. Same semantics the planner note documents. No gap.
- **Security scout: 1 finding = documented design, DROPPED.** `checkDailySpendCeiling` is check-then-act (read, then `recordDailyExport` AFTER success) vs the atomic incr-first `enforceGenerationCeiling`. This is DELIBERATE + documented: the daily EXPORT ceiling (50/day fair-use) counts SUCCESSFUL exports; the GENERATION ceiling (the real wallet-drain guard) is atomic incr-first (spend-ceiling.ts:250). The "race" allows ≤1 surplus successful export at the boundary on a fair-use cap; converting to incr-first would make FAILED exports burn the daily cap (semantic regression on the critical path). The authoritative wallet backstop is already atomic. Not a bug.
- **Design/a11y scout: 4 findings ALL FALSE ALARMS.** Flagged EditorStep/ExportStep buttons "missing focus rings" — but `globals.css:116 button:focus-visible` gives EVERY `<button>` an outline globally (that's why #529 targeted `<a>` links specifically). Covered. Do NOT add per-button focus-ring classes.
- **Artifact-freshness scout: ZERO findings** (pricing/quota/animations/filters all reconcile; prior overclaims already fixed).
- **Test-coverage dead-code drops:** `poll-manager.ts:99-112` consecutive-poll-error branch — `pollBatched` callers are style-transfer (ExportStep, not v1) + DetectingStep animate (uncertain v1) → dead-code risk, skipped. `beat-sync.ts:55 nextBeatAfter` fallback — the function has NO production caller (grep: only its own definition), a dead export → skipped. `velocity.ts:100` single-keyframe guard — no confirmed production caller passes a single-keyframe array (speculative reachability) → skipped as potential padding.

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (coverage/test-robustness completes no DoD/Track item); NO business-case recompute (no lever/pricing/COGS change). Quiet coherent run — 3 shipped, 1 in-PR padding test dropped on a genuine Reviewer-B catch, + a reusable env-quirk + do-not-repropose fact.

## Run 81 — 2026-07-17 — 2 merged PRs (#534 transitions custom-color coverage · #535 kinetic-text Neon font coverage) + 1 ABANDONED (#533 COGS validate-cache — Haiku-4.5 4096-token cache floor) — no deep audit (Run 80 ran one 2026-07-17, <24h)
Cold start; synced to `origin/main` (tip 81d5d0e) FIRST. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac), **functional_reality B** (overlay-pass export test hangs on CI simulator = iOS/can't-verify; export-COUNT quota client-side = iOS), **tests_evals B** (real paid-AI round-trip / `ios` → REQUIRED = owner key + `.github` edit I'm barred from)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline green (npm ci + build + test + lint exit 0; 1163 tests pre-change). Full 6-Haiku scout sweep + VERIFIED every finding myself before acting. Full 2-Sonnet-reviewer gate per change; multi-branch disjoint fan-out worked. **maker≠checker EARNED ITS KEEP this run — Reviewer B killed the run's anchor change on a real, doc-verified defect (see #533).**

**What shipped (2 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#534 (Track G, coverage):** the AI custom-color branches of `drawTransitionOverlay` (glitch RGB bands `transitions.ts:264-276` + light_leak gradient `:308-312`). `renderOptions.glitchColors`/`.lightLeakColor` flow from `productionPlan` into the export/preview render (ExportStep/DetectingStep/TapePreviewPlayer) — planner-reachable, not dead. Only defaults were exercised. 2 non-tautological tests (assert AI tint reaches every band/stop AND defaults don't leak). 23→25.
- **#535 (Track G, coverage):** the `Neon` arm of `drawKineticCaption`'s per-style font switch (`kinetic-text.ts:397`), the one untested case (Bold/Minimal/Classic were covered). Neon is template-selectable (`templates.ts:43,73`) + allow-listed (`detect.ts:3042`) → reachable. One assertion on the existing test.

**ABANDONED — #533 COGS "cache the validate system prompt" (maker≠checker + doc verification):** the run's intended anchor. Added `cache_control:{type:"ephemeral"}` to the `/api/validate` Haiku system prompt to match the scorer/planner. Reviewer A APPROVE (noted the token-floor as non-blocking); **Reviewer B REQUEST_CHANGES**, correctly: `CLAUDE_VALIDATOR` = **Haiku 4.5**, whose **minimum cacheable prefix is 4096 tokens** (I then VERIFIED against Anthropic's prompt-caching docs, 2026-07: Haiku 4.5 = 4,096). The validator system prompt is only ~1.2k (no-frames) to ~2.6k (frames) tokens — BELOW the floor — so `cache_control` is silently ignored (no cache read, no error) and the COGS benefit CANNOT fire. Shipping a marker + a comment asserting a saving that never materializes fails the value bar. Closed PR clean; local branch deleted (remote delete 502'd — harmless, PR closed).

**KEY FINDING / DO-NOT-REPROPOSE — Haiku 4.5 prompt-cache floor = 4096 tokens.** Both the validator (~1.2-2.6k) AND the existing SCORER system prompt (`detect.ts:1042`, ~2.3k tokens) sit BELOW it, so the scorer's `cache_control` ALSO never fires — adding `cache_control` to ANY Haiku-4.5 call whose system prompt is <4096 tokens is a NO-OP. Do NOT re-propose "cache the validate/scorer system prompt" as-is. (Sonnet-4.6 planner at 1024-min IS above its floor and DOES cache — leave it.) **POTENTIAL FUTURE COGS LEVER (eval-gated, deferred):** padding the SCORER system prompt to ≥4096 tokens (e.g. a richer few-shot rubric) would let calls 2-4 of the 4 batched scorer calls cache the shared prefix → a real per-export COGS save on the HIGHEST-VOLUME paid call. BUT it's a core-detection-path change requiring G3 eval evidence that the richer prompt doesn't shift scoring quality (keyless-blocked from Linux). Record in docs/MODEL_COSTS.md if ever pursued.

**DROPPED — verified, not shipped (do NOT re-propose without the named precondition):**
- **COGS validate-prompt over-serialization** (trim per-clip rendering params from the validator prompt): behavior change to what Haiku sees on the critical export path; no keyless way to prove validation quality holds → risky, deferred.
- **COGS planner condensed score-summary** (send top-10/source instead of all scored frames): changes the planner's INPUT → could change plan/clip selection; core-detection-path, needs real-AI eval evidence. Deferred.
- **store.ts `SET_TALKING_HEAD` reducer + poll-manager.ts:96 `"Task failed"` fallback** (test-coverage scout): both serve the AtlasCloud talking-head/style-transfer routes, which are **"not enabled in v1"** (press-kit) → dead-code coverage in v1, same class Run 80 rejected for the atlascloud `generate*` wrappers. Add alongside enabling those features.
- **a11y aria-hidden on decorative icons** (ResultsStep/Header, 5-6 lucide icons): the repo has NO aria-hidden-on-icons convention outside `Confetti.tsx`; lucide renders title-less `<svg>` that SRs largely don't announce; a partial 4-5-icon sweep is inconsistent churn and a full sweep across all step components is large/low-value; the back buttons already carry `aria-label`s → no actual a11y BUG. The Award "#1 badge" is icon-only (conveys rank) so hiding it would REMOVE meaning. Dropped clean (no commit). Revisit only as a full, coherent a11y pass if the app-shell ever adopts the convention.
- **a11y tap-target resize** (back/dismiss buttons h-9/h-8 → h-11): already WCAG-2.5.8-AA compliant via the global `(pointer:coarse) min-height:44px` rule; bumping to 44×44 is a debatable AAA-only visual change on secondary nav. Dropped.
- **growth UTM-attribution / referral-loop / experiment-engine landing wiring / pricing-copy reframe:** pre-PMF (0 traffic) → PRODUCT bias holds; referral bakes in an unapproved "2 months Pro free" monetization promise; experiment-wiring + copy are agent-authored marketing on a surface owned by the separate Growth Agent routine; several scout citations were fabricated/placeholder links. Defer to the growth routine / post-launch.
- **Security + artifact-freshness scouts: ZERO fresh findings** this sweep (security re-confirmed the full Track-H stack on all ~26 paid routes; all marketing/ASO/pricing claims reconcile with code).

Scorecard/blockers UNCHANGED; NO ROADMAP box ticked (coverage hardening completes no DoD/Track item); NO business-case recompute (the one COGS candidate was a no-op, abandoned — no lever/pricing/COGS actually changed). Quiet coherent run — 2 shipped, 1 abandoned (genuine Reviewer-B catch), and the abandonment produced a reusable do-not-repropose fact + a named future lever.

## Run 80 — 2026-07-17 — 3 merged PRs (#529 a11y focus rings · #530 security H2 vision+planner body cap · #531 waitlist-store KV coverage) + DEEP AUDIT 2026-07-17
Cold start; synced to `origin/main` (tip a4e1bec) FIRST. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac), **functional_reality B** (overlay-pass export test hangs on CI simulator = iOS/can't-verify; export-COUNT quota client-side = iOS), **tests_evals B** (real paid-AI round-trip / `ios` → REQUIRED = owner key + `.github` edit I'm barred from)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline green (npm ci + build + test + lint exit 0). Multi-branch disjoint fan-out worked (per-change branch pushes fine this run). Full 2-Sonnet-reviewer gate per change; maker≠checker caught two real defects (see below).

**DEEP AUDIT — 2026-07-17 (Run 80) — 7 read-only Haiku lenses** (test-coverage, Track-H security, correctness/functional-reality, COGS/model/perf, design-taste/a11y, artifact-freshness/honesty, growth/business-case). ~2 days since Run 74's full audit. Distilled → the 3 shipped changes below + verified drops. Every finding I VERIFIED myself before acting.

**What shipped (3 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#530 (Track H2, security — the run's anchor):** pre-parse `enforceBodyLimit` on the 6 paid vision/planner routes that called `req.json()` with no Content-Length guard (the audio/image routes already had it). Two caps: 300MB vision (score/ios-score ≤1000 frames; validate/ios-validate ≤MAX_FILES clipFrames) + 3GB planner (plan/ios-plan ≤MAX_PLANNER_FRAMES=12,000). **maker≠checker WORKING:** first single-300MB attempt was reviewer-REJECTED (both reviewers, independently) — 300MB would 413 a legit large `/api/plan` project (12,000 frames ≈1.9GB); ios-plan was also omitted. Re-scoped to two per-route caps + added ios-plan; re-review (cycle 2) both APPROVE, then adopted their endorsed 2GB→3GB headroom bump. **This DONE work supersedes the "frame-route body cap = deferred/no-safe-cap" notes below (Runs ~76-78) — retired inline.**
- **#529 (Track A5/D, a11y):** `focus-visible` rings on the non-highlight pricing CTA + all privacy/terms/support inline links (plain `<a>`, no `a:focus-visible` fallback in globals.css). Reuses the shipped footer/nav token. Reviewer B (cycle 1) caught 2 missed `text-sm` support links → fixed before merge.
- **#531 (Track G, coverage):** KV branch of `addConfirmedSignup` + `getWaitlistCounts` (`?? 0` null-coalescing) in waitlist-store.ts (E6a) — reachable production (called from `/api/waitlist` + `/api/growth/stats`), previously in-memory-only. Branch 72.7%→~100%.

**ABANDONED — atlascloud generateLipSync/generateStyleTransfer coverage (dead-code, maker≠checker caught it):** initial 3rd change tested the one-shot `generate*` wrappers in atlascloud.ts. Reviewer B REJECTED: those wrappers have NO production callers — the talking-head/style-transfer ROUTES call the `submit*` halves directly (submit-on-server → poll-from-client, maxDuration=60). Testing them = coverage on dead code, fails the value bar. Correct call. PIVOTED the 3rd change to waitlist-store KV (reachable) → #531. Reason: `review_value` / dead_code. (My selection error: verified "uncovered" but not "reachable" before writing the test.)

**DROPPED — verified false alarms / known dead-ends (do NOT re-propose without the named precondition):**
- **planner-timeout (`detect.ts fetchWithRetry` w/o timeoutMs)** — correctness scout re-flagged the twice-reverted dead-end. `AbortSignal.timeout` binds the WHOLE streaming body; the planner legitimately runs 1-3min near the 300s ceiling, so any whole-fetch timeout clips healthy streams (reverted `26db6ca`, #184, reviewer-rejected Runs 36/78). The ONLY residual is a headers-phase-only connect timeout on shared `fetchWithRetry` (blast radius, streaming semantics, not Linux-verifiable) — low value, still deferred. DO NOT re-propose the naive timeoutMs.
- **ios-score prompt-cache "static type" (COGS scout)** — claimed the 4 batched score calls re-pay the system prompt because `cache_control:{type:"ephemeral"}` "only works within one request". FALSE: ephemeral caching persists ~5min ACROSS requests, and the 4 batches are near-simultaneous with an identical system prefix → calls 2-4 hit the cache. Anthropic has NO "static" cache type. Already optimized.
- **AppStoreMetadata "Premium filters & effects for Pro subscribers" (artifact scout)** — alleged overclaim. FALSE: `PremiumEffectRenderer` IS Pro-gated (`EditorView.premiumEffectsButton` routes non-Pro to the paywall; `ExportView:289` applies `selectedPremiumEffects`). The claim is ACCURATE; the only real inconsistency (landing page omits it from the Pro list) is not a contradiction/trust-risk. Leave as-is.
- **growth levers (activation event, paywall A/B, churn tracking, credit-pack UI)** — all 4 growth-scout candidates are iOS-gated (AnalyticsService.swift / PaywallView.swift / StoreKit, can't verify from Linux) OR premature (pre_launch, 0 traffic → nothing to measure). PMF-leading-indicator says bias PRODUCT pre-PMF; adding unwired events with no traffic is low value. Defer to post-launch / owner iOS work.

Scorecard/blockers unchanged; NO ROADMAP box ticked (H2 is real hardening but no DoD/Track box completes on it alone); NO business-case recompute (no lever/pricing/COGS change). Quiet coherent run — 3 shipped, 1 abandoned (dead-code, pivoted), 2 review-cycle catches.

## Run 79 — 2026-07-16 — 2 merged PRs (#526 social-queue KV-path coverage · #527 features_view mid-funnel instrumentation) — no deep audit (Run 74 ran one 2026-07-15 + #506 DEEP AUDIT 2026-07-15, <24h; this run's 6-Haiku sweep is the audit-equivalent)
Cold start; synced to `origin/main` (tip e6156a6) FIRST. Consumed QUALITY_SCORECARD (as_of 2026-07-15, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C** (archivable Xcode target + 6.9" screenshots = owner Mac work), **functional_reality B** (overlay-pass export test hangs on CI simulator = iOS/can't-verify; export-COUNT quota client-side = iOS), **tests_evals B** (promote a real paid-AI round-trip / `ios` to REQUIRED = owner-funded key + a `.github` edit I'm blast-radius-barred from)). GROWTH_STATUS pre_launch, funnel/pmf 0/null (bias PRODUCT; no acquisition scaling). BUSINESS_CASE honest, base y1 $7,740, floor ~month-38 — all consumed as DATA. Web baseline green (npm ci + build + test + lint exit 0). Full 6-Haiku scout sweep (test-coverage, Track-H security, functional-reality/entitlement, COGS/reliability, marketing/design-taste, artifact-freshness) + VERIFIED every finding myself before acting. Full 2-Sonnet-reviewer gate per change; both dual-APPROVED; multi-branch disjoint fan-out (pushes to per-change branches worked fine this run).

**What shipped (2 PRs, 2-reviewer + all required checks green, auto-merged):**
- **#526 (Track G, coverage):** covered the durable Vercel-KV branch of `social/queue.ts` (`enqueue` rpush + `listQueue` string/object deserialization, `queue.ts:74-76`/`85-88`) — mock `@vercel/kv`, inert for the in-memory suite. Reviewer A mutation-tested it non-tautological. 74.28%→91.42% stmts, 60%→85% branch, lines→100%. Pure test.
- **#527 (Track E, growth):** `features_view` IntersectionObserver event on the landing Features section — the missing mid-funnel step between `waitlist_form_view` (hero) and `pricing_view`. Instrumentation-only, mirrors the shipped `pricing_view` observer; no UI/CTA change; no fabricated metrics (emit-point only, real numbers need Plausible = owner).

**DROPPED — verified false alarms / speculative (do NOT re-propose without the named precondition):**
- **`/api/render` Track-H guards** (security scout's only finding): route returns 501 at `render/route.ts:76` BEFORE any body parse when `RENDER_ENABLED!="true"`, AND even when enabled returns 501 again at :114 without EVER calling a paid provider (the FFmpeg render worker is not deployed). So rate-limit/entitlement/spend-ceiling guards would be UNREACHABLE dead code on a route that does zero paid work → speculative hardening of an unbuilt feature. Add the guards WHEN the render worker actually lands, not before.
- **Caption-style doc "fix"** (artifact-freshness scout: press-kit/aso "pop, bounce, slide, typewriter" allegedly wrong): **FALSE ALARM** — scout conflated `CaptionStyle` (Bold/Minimal/Neon/Classic *visual* styles; web `kinetic-text.ts` maps Neon→flicker) with `KineticCaptionStyle` (`Sources/Models/ViralEditConfig.swift:42-67` pop/bounce/slide/typewriter/flicker — the *animation* types). `KineticCaptionRenderer.swift` delivers real bounce/pop/slide/typewriter animations; the marketing claim is ACCURATE for the shipped iOS app. "Fixing" the docs would introduce a WRONG claim. Leave as-is.
- **functional-reality/entitlement "check-without-consume"** (re-verified AGAIN, still not a bug): 15 sub-routes call `checkExportAllowed()` but only `/api/score` + `/api/ios-score` call `consumeExport()` — intentional (one export = one consume at score; sub-steps check-only so a user can't hammer them quota-free). Monthly free quota is KV-atomic + server-authoritative + fail-closed. Don't re-flag.
- **frame-extractor.ts:116 / poll-manager.ts:72 fetch timeouts** (COGS/reliability scout): DEFER — client-side BROWSER fetches; frame-extractor's `videoUrl` is typically a local blob (resolves instantly), poll-manager already has a deadline check (`poll-manager.ts:58`). Marginal value, not the serverless-timeout rule.
- **sfx-library.ts fuzzy-match coverage (63%/41% branch):** DEFER — the LIBRARY fuzzy path is DEAD until the owner populates CDN URLs (every entry `url: null`); covering it needs a testability refactor of dormant code (borderline value-bar). Add tests alongside the URLs when they land. (Chose `social/queue.ts` KV coverage instead — reachable production code, no refactor.)

**FOLLOW-UP candidate (Reviewer B, non-blocking):** `LandingPage` now has THREE near-identical IntersectionObserver view-event effects (features/pricing/waitlist). A shared `useSectionViewEvent(ref, event, threshold)` hook would dedupe — but a dedup-only refactor is often churn; only worth it if a 4th observer is added (raise the value then).

Scorecard/blockers unchanged; NO ROADMAP box ticked (nothing DoD-level completed); NO business-case recompute (no lever/pricing/COGS change). Quiet coherent run.

## Run 78 — 2026-07-16 — 1 merged PR (#521 spend-ceiling fail-closed coverage) — no deep audit (Run 74 ran one 2026-07-15, <24h)
Cold start; synced local branch to `origin/main` (tip 77ae45e) FIRST. Consumed QUALITY_SCORECARD (as_of 2026-07-15, commit a754876, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C, functional_reality B, tests_evals B**, all un-closeable from Linux: archivable Xcode target + 6.9" screenshots (owner Mac), the overlay-pass export test that hangs on the CI simulator (iOS, can't verify), and promoting a real paid-AI round-trip / `ios` to a REQUIRED check (owner-funded key + a `.github` edit I'm blast-radius-barred from). GROWTH_STATUS (pre_launch, funnel/pmf 0/null → bias PRODUCT), BUSINESS_CASE (base y1 $7,740, floor ~month-38, honest) all as DATA. Web baseline green (npm ci + build + test + lint, exit 0; 1147 tests pre-change, 0 lint, coverage ~94.2/86/93/95.6). Ran a full 6-Haiku read-only scout sweep (security/Track-H, backend correctness/timeouts, test-coverage, COGS/model/perf, artifact-freshness/honesty, web design-taste/a11y) + VERIFIED every finding myself before acting. Full 2-Sonnet-reviewer gate.

**BRANCHING CONSTRAINT this run:** pushes to arbitrary per-change branch names return HTTP 403 in this env, and a stale remote `claude/planner-fetch-timeout` @943dbad already existed (likely a concurrent sibling run) — so I consolidated onto the designated branch `claude/wizardly-albattani-yusawy` (one PR) instead of the usual multi-branch disjoint fan-out.

**What shipped (1 PR, 2-reviewer + all 4 required checks green, auto-merged 11:15):**
- **#521 (Track G, coverage):** locked the fail-closed `catch` of the exported `checkDailyGenerationCeiling` (deny with `usage===cap` on a KV read throw) — the sole uncovered line in `spend-ceiling.ts` (line 226); sibling `checkDailySpendCeiling`'s fail-closed path was tested, this wasn't. Pure test. cov 96.92%→98.46%.

**REVIEWER-REJECTED + dropped (maker≠checker working):** the PR originally also added `280_000` timeout to the streaming planner `fetchWithRetry` in `detect.ts`. Reviewer A (correctness) REJECTED: `AbortSignal.timeout` binds to the WHOLE streaming body (not just connection setup), and the planner legitimately runs 1–3 min near the 300s ceiling (plan/route.ts:10; max_tokens raised 32k→64k for adaptive thinking), so 280s could clip a healthy near-ceiling stream — and the abort propagates UNCAUGHT out of the `PLANNER_EMPTY_RETRIES` loop (no try/catch around `consumeSSEStream`) → generic "Planning failed" with NO retry, contradicting the "clean retryable" claim. Correct call; dropped via force-with-lease before merge. My selection error: I under-weighted that the planner alone can run near the full 300s budget.

**FUTURE-RUN FINDING — planner pre-headers hang (real gap, needs a careful fix):** the `detect.ts` planner streaming fetch is the ONE external call with NO connection-phase timeout — a TCP/TLS hang BEFORE response headers arrive is unguarded by `consumeSSEStream`'s per-chunk `STREAM_READ_TIMEOUT_MS` (90s, only starts once body reads begin) → hangs to the 300s platform kill. CORRECT fix = a headers-phase-ONLY timeout: a manual `AbortController` armed via `setTimeout`, CLEARED right after `await fetch()` resolves (headers received) so the streaming body is NEVER subject to it — NOT `AbortSignal.timeout` (whole-lifecycle) and NOT any whole-request cap (clips healthy long streams). Touches shared `fetchWithRetry` (blast radius across all callers incl. non-streaming), so it needs its own branch + full review; low priority (modest value — only converts an opaque platform kill into a clean error on a rare hang).

**DEFERRED scout findings (verified real, not shipped this run — pick up later):**
- **Frame-route pre-parse body cap** (score/plan/validate/ios-score/ios-plan/ios-validate): **[SUPERSEDED — DONE Run 80 / #530.]** This was resolved exactly as the "correct fix" below predicted: a GENEROUS per-route Content-Length cap above realistic traffic but below the GB bomb — 300MB vision (count-bounded ≤1000/≤MAX_FILES routes) + 3GB planner (≤MAX_PLANNER_FRAMES=12,000). No new aggregate-bytes bound was needed; the existing per-count × per-frame caps make the generous cap safe (verified by 4 reviewers). Do NOT re-flag as deferred/unsafe. Original note: the `enforceBodyLimit`/`JSON_BODY_LIMIT_BYTES` (1MB) helper is for TEXT-only routes; these are FRAME routes whose authoritative per-field envelope is huge (score accepts up to 1000 frames × `MAX_FRAME_B64_CHARS` 12M ≈ 12GB), so NO tight pre-parse Content-Length cap is safe WITHOUT also tightening the aggregate frame bound (a semantic change to the accepted-frames envelope). A wrong cap breaks the core detection path (BUILDS≠WORKS) → deferred. Correct fix = add an aggregate-bytes/aggregate-frames bound, THEN a generous Content-Length cap above realistic traffic (~tens of MB) but below the GB bomb.
- **Validation second-pass skip (COGS):** `validate/route.ts` computes a `qualityScore` but doesn't return it; the client (`DetectingStep.tsx`) always runs up to 2 passes. Returning issue-count/qualityScore + skipping pass 2 when `issues≤1` AND no expensive regenerations would save ~1–2 Haiku vision calls on ~5–15% of exports (~$0.01–0.03/export when it applies). Behavior change on the export path + a client change → needs careful design; deferred.
- **rate-limit `E2E_RATELIMIT_BYPASS` return-shape test:** marginal (CI-only, zero prod risk) — not shipped.

Scorecard/blockers unchanged; NO ROADMAP box ticked (nothing DoD-level completed); NO business-case recompute (no lever/pricing/COGS change). Quiet coherent run.

## Run 77 — 2026-07-16 — 3 merged PRs (#515 H2 intro/outro bounds-first · #516 kinetic glow/hexToRgba coverage · #517 atlascloud terminal-branch coverage) — no new formal deep audit (Run 74 ran one 2026-07-15, ~3 runs/<24h; this run's 6-Haiku sweep is the audit-equivalent)
Cold start; synced local branch to `origin/main` (tip ffee487) FIRST, branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, commit a754876, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C, functional_reality B, tests_evals B**; the Linux loop cannot close any of them — archivable Xcode app target + 6.9" screenshots (owner Mac), the overlay-pass export test that hangs on the CI simulator (iOS, can't verify), and promoting a real paid-AI round-trip / `ios` to a REQUIRED merge check (needs owner-funded ANTHROPIC key + a `.github/ci.yml` edit I'm blast-radius-barred from). No ROADMAP box ticked this run — none of the three changes completes a ship-critical item). GROWTH_STATUS (pre_launch, funnel/pmf 0/null → bias PRODUCT), BUSINESS_CASE (base y1 $7,740, floor ~month-38, honest) all as DATA. Web baseline green (npm ci + build + test + lint, exit 0; 1140 tests, 84 files, 0 lint, coverage 93.17/84.68/92.67/94.59). Ran a full 6-Haiku read-only scout sweep (security/Track-H, backend correctness/timeouts, test-coverage, artifact-freshness/honesty, monetization/revenue-levers, design-taste on web surfaces) + VERIFIED every finding myself before acting. Full 2-Sonnet-reviewer gate per change (worktrees for the two in-flight test changes to avoid the reviewer-checkout HEAD-move hazard; reviewers given committed-ref diffs).

**What shipped (3 PRs, 2-reviewer + all required checks green):**
- **#515 (Track H, security):** `/api/intro` + `/api/outro` ran `checkExportAllowed` (ES256 JWS verify when a Pro `signedTransaction` is present + a KV quota read) BEFORE bounding the prompt — the reverse of every sibling paid route. Moved the type/length validation ahead of the gate (H2 bounds-before-expensive-work) + an ordering-regression test per route asserting `checkExportAllowed` is not invoked on an oversized prompt. Only intro/outro were genuinely reversed (see verified-drop on validate below).
- **#516 (Track G, coverage):** the private `hexToRgba` (caption glow color math: hex→rgba / rgb passthrough / white fallback) + the two-layer glow render pass in `drawKineticCaption` had ZERO direct coverage — a regression ships the wrong glow color into every export silently. Extended the canvas mock to record the `shadowColor`/`shadowBlur` assignment HISTORY (the main pass overwrites the glow color, so only a history proves the glow layers ran) + 4 tests.
- **#517 (Track G, coverage):** `checkTaskResult` (AtlasCloud video-gen poll) terminal branches — the `completed` success alias (a finished render reported as `completed` not `succeeded` would fall to `processing` and hang the poll until timeout), a `canceled` task (must resolve to `failed`, not strand), and the `error ?? "unknown error"` fallback (avoid `Task failed: undefined`). +3 direct tests. Note: the "succeeded-but-empty-outputs" case was ALREADY covered (atlascloud.test.ts:551) — did not re-add.

**Scout findings VERIFIED and DROPPED this run (record so future runs don't re-chase):**
- **"bounce" caption marketing claim = NOT an overclaim (DROPPED).** press-kit.md/aso-package.md name "pop, bounce, slide, typewriter animations" — these are EXACTLY the 4 KineticCaptionStyle cases that ANIMATE (KineticCaptionRenderer.swift: pop/bounce/slide/typewriter get dedicated `add*Caption` animations; flicker+fade render via `addStaticCaption` — TODO, so they DON'T animate). The docs correctly list the animating subset and omit the two static ones. The web planner `detect.ts` caption allow-list omits "bounce" (6 styles: pop/slide/flicker/typewriter/fade/none) — a real iOS↔web parity gap, but NOT a bug (the planner emitting a subset is fine) and it touches the plan→iOS render string contract I cannot verify on Linux. Left alone. Do NOT "fix docs" (they're honest) and do NOT add "bounce" to the web allow-list without verifying the iOS decode mapping on a Mac.
- **KV-backed rate limiter (security A→A+ gap) = DEFERRED (blast radius).** `rate-limit.ts` uses an in-memory `Map` per serverless instance; making it cross-instance means converting `checkRateLimit` sync→async, which touches EVERY route handler (non-file-disjoint, high blast radius) — and the scorecard itself flags it as defense-in-depth ONLY (the authoritative wallet guards — server-verified entitlement + KV-atomic fail-closed spend ceilings — are already cross-instance, so paid-spend blast radius is hard-bounded regardless). Not worth the sprawl for an A→A+ nicety.
- **`validate/route.ts` is NOT reversed (FALSE POSITIVE).** A naive "checkExportAllowed-before-a-`.length`-check" grep flagged it, but it bounds all prompts via `overStringLimit(...)` at :80-88 BEFORE `checkExportAllowed` at :96; the :118 `.length` the grep caught is unrelated. Only intro/outro were genuinely reversed. Do NOT reorder validate.
- **Monetization levers (frame/planning cross-user cache · 7-day free trial · higher creator tier · affiliate · re-engagement email · admin billing dashboard) = NONE BUILT (premature / owner-blocked / wrong-arch for a pre-PMF Linux-only run).** Cross-user frame/plan cache gives near-ZERO real dedup on PERSONAL video (different users don't upload identical footage) — speculative infra + new paid-endpoint attack surface. A "7-day free trial" should be a StoreKit INTRODUCTORY OFFER on the subscription (owner App Store Connect config), NOT a custom server endpoint that grants Pro entitlement (bypasses StoreKit, App-Store-rejection risk, un-convertible). Higher tier / affiliate are premature pre-PMF and are iOS/`.storekit`/UI builds. Re-engagement email is blocked on the Resend connection (owner, PENDING_OPS gtm-connect-email). Admin dashboard is post-launch (needs live traffic). The credit-pack lever remains the standing `business_case_strength` gap (backend done; `.storekit` consumable SKU + iOS StoreKit purchase UI missing — owner/iOS). Do NOT build a web "Buy exports"/trial button that can't actually transact (fake/dead button, BUILDS≠WORKS).
- **Design-taste + backend-correctness sweeps = CLEAN.** Design scout: web surfaces show intentional, iterated taste (custom tokens, lucide icons, coherent type scale) — no generated-looking findings. Correctness scout: every external/LLM call has a timeout shorter than the serverless budget; JSON.parse guarded; quota/period math correct — no real defect.

### What NOT to re-do (additions for Run 77)
- Do NOT "fix" the press-kit/aso "bounce" caption claim — it's honest (bounce is one of the 4 animating styles); flicker/fade are the two that render static.
- Do NOT add "bounce" to `detect.ts`'s web planner caption allow-list without Mac-verifying the plan→iOS `KineticCaptionStyle` decode mapping (iOS enum rawValues are capitalized display strings, web uses lowercase — a mapping layer exists but is unverifiable on Linux).
- Do NOT reorder `validate/route.ts` — it already bounds prompts before `checkExportAllowed` (:80-88 before :96); the reorder-grep flagged an unrelated `.length` at :118.
- Do NOT convert `rate-limit.ts` to KV-backed — sync→async touches every route (non-disjoint), and it's defense-in-depth only (wallet guards already cross-instance).
- Do NOT build web-side monetization levers (cross-user frame/plan cache, custom free-trial endpoint, credit-pack/trial buy buttons) — near-zero-value / wrong-arch / fake-button for this pre-PMF Linux loop; the real revenue path is iOS StoreKit (owner-blocked). A free trial belongs in App Store Connect as an intro offer, not a server entitlement bypass.
- Do NOT re-add an atlascloud "succeeded-but-empty-outputs" test — already covered (atlascloud.test.ts:551).

## Run 76 — 2026-07-15 — 3 merged PRs (#512 provider-throw hygiene ×5 routes · #513 waitlist_form_view funnel · #511 voice-clone throw-hygiene) — no new deep audit (Run 74 ran one 2026-07-15, <24h)
Cold start; synced local `main` (was stale at 8b0b04b) to `origin/main` (tip 1b27872) FIRST, branched each change off it. Consumed QUALITY_SCORECARD (as_of 2026-07-15, commit a754876, overall B, ship_gate_met=false — SAME 3 owner/iOS/CI-blocked ship-critical dims: **store_readiness C, functional_reality B, tests_evals B**; the Linux loop cannot close any), GROWTH_STATUS (pre_launch, funnel/pmf 0/null → bias PRODUCT + build the growth-analytics surface), BUSINESS_CASE (base y1 $7,740, floor ~month-38, honest) all as DATA. Web baseline green (npm ci+build+test+lint, exit 0; 1132 tests, 82 files, 0 lint). Last DEEP AUDIT was Run 74 (2026-07-15, <24h) → NO new deep audit; ran a full 7-Haiku read-only scout sweep (security/Track-H, backend correctness/timeouts, test-coverage, artifact-freshness/honesty, design-taste on web surfaces, COGS/revenue-levers, growth/ASO/SEO) + VERIFIED every finding myself before acting. Full 2-Sonnet-reviewer gate per change.

**What shipped (3 PRs, 2-reviewer + all required checks green):**
- **#512 (Track H/G, test):** provider-EXCEPTION (throw) error-hygiene coverage on 5 paid routes (voiceover/sfx/music/intro/outro). The existing resolve-path suite only mocks `{status:"failed"}` (→502); the orthogonal throw→generic-500 `catch` branch was uncovered — a future `err.message` interpolation would leak the vendor + upstream status (H3). New `provider-exception-hygiene.test.ts` asserts generic 500 + no vendor/status leak; verified falsifiable via a leak-mutation on the intro route.
- **#513 (Track E/GTM, growth analytics surface E7):** `waitlist_form_view` fired once when the hero waitlist form scrolls into view — the missing form-impression step between Plausible `page_view` (denominator) and `waitlist_signup` (numerator). Distinguishes a below-the-fold visibility problem from a weak-copy conversion problem — a diagnostic `page_view` (fires on every load regardless of scroll depth) cannot give. Mirrors the existing `pricing_view` observer; hero-only (no double-count); no `landing_view` (redundant with Plausible). Instrumentation only.
- **#511 (Track H/G, test):** the voice-clone provider-throw hygiene guard — the one paid route #512's JSON-body sweep can't reach (it's multipart `formData()`). Its throw→500 branch had zero coverage (pr61-routes.test.ts covers only 413/400/402/200). One multipart test; verified failing on a leak mutation; rebased onto main so the referenced sibling file is present.

**maker≠checker SAVE this run (verification working):**
- **#511 draft was padded** — first version added 7 tests + a `name`-length bound. Reviewer B correctly caught that 4-5 tests DUPLICATED pr61-routes.test.ts (which already covers voice-clone's 413/400/402/200/200-success) and my "previously untested" claim was FALSE, AND that the `name` bound guarded a NON-cost-vector: `elevenlabs-voice-clone.ts` does `formData.append("name", name)` — ElevenLabs stores it as voice metadata, NOT interpolated into any prompt (unlike ios-score's `templateName`), and the 10MB audio blob (already capped) is the real cost driver; the bound also used a local constant + silent-truncate vs the shared `input-bounds.ts` reject-with-413 convention. Dropped the name bound + all duplicate tests; slimmed to the one genuinely-novel throw-path guard. Re-reviewed clean.

**Scout findings VERIFIED and DROPPED this run (record so future runs don't re-chase):**
- **`spend-ceiling.ts recordDailyGeneration` missing try-catch = DROPPED (dead-path).** Grep shows the function is called ONLY in tests, never on any production route (enforceGenerationCeiling does admission-time atomic INCR before the paid call; there is no post-call recordDailyGeneration site). The "user gets a 500 after a charged success" scenario cannot occur → adding the try-catch is churn on an unused path.
- **`music/submit` durationMs unbounded = FALSE POSITIVE.** `elevenlabs-music.ts:57-58` already clamps: `safeDuration = Number.isFinite(durationMs) ? durationMs : DEFAULT`, then `Math.round(Math.max(3_000, Math.min(safeDuration, MAX_MUSIC_LENGTH_MS)))` — Infinity/NaN/huge all bounded internally. No gap.
- **`poll-manager.ts` `results.indexOf(result)` → direct index = DROPPED (not a real bug).** The scout itself conceded "tests pass, code works in practice"; PromiseSettledResult objects are distinct references so indexOf returns the correct index. Objectively-nicer direct-index is churn on an A-graded module. Skip.
- **Design: PRO-badge `purple-500/pink-500` → accent tokens (UploadStep.tsx) = DROPPED (borderline lateral).** Real minor off-brand hue (purple-500 #a855f7 vs --accent #7C3AED) + a 2× duplicated inline badge, but on an A+ design surface a hue swap risks reading as lateral restyle; the subtle inline treatment is a deliberate distinction from the filled `bg-accent-gradient` "Most Popular" hero badge. Discipline over padding — not shipped.
- **Growth: canonical URLs on policy pages, BreadcrumbList/NewsletterSignup JSON-LD, root `/`→/landing canonical, confirm-page→page.tsx conversion = DROPPED** as BORDERLINE/low-pre-launch-impact (and the confirm→page conversion would move the confirmation URL out of /api and break existing links). Artifact-freshness scout found ZERO doc-vs-code contradictions (pricing/quota/models/features all consistent).
- **COGS/revenue web-paywall levers (regen cap, credit-pack UI, annual-tier hierarchy in ExportStep/EditorStep) = DROPPED** — these touch the web app step components whose purchase/planner paths are the iOS-real revenue path this Linux loop can't verify; adding a "Buy exports" button on web that can't actually transact would be a fake/dead button (BUILDS≠WORKS). Better as owner-blocked iOS work. The credit-pack lever remains the named business_case_strength gap (iOS/owner-blocked: backend done, .storekit consumable SKU + StoreKit purchase UI missing).

### What NOT to re-do (additions for Run 76)
- Do NOT re-add voice-clone guard/resolve tests (413/400/402/200) — already in pr61-routes.test.ts; only the throw path (#511) was the gap.
- Do NOT re-add a voice-clone `name`-length bound — `name` is ElevenLabs FormData METADATA, not prompt-interpolated; non-cost-vector; the 10MB audio cap is authoritative.
- Do NOT add a try-catch to `spend-ceiling.ts recordDailyGeneration` — the function is test-only/unused; dead-path churn.
- Do NOT "fix" `music/submit` durationMs — already clamped in elevenlabs-music.ts (Math.max(3000, Math.min(…, MAX)), NaN/Infinity guarded).
- Do NOT swap poll-manager `indexOf` to direct index — not a real bug (distinct settled-result refs), churn on an A module.
- Do NOT swap the PRO-badge purple-500 → accent tokens — borderline lateral on an A+ surface; deliberate distinction from the filled Most-Popular badge.
...
- Do NOT re-flag the 6 paid routes (voiceover/sfx/intro/outro/music·submit/stems) as missing a pre-parse body cap — done #504 (`enforceBodyLimit` + `JSON_BODY_LIMIT_BYTES`). **[UPDATE Run 80: the frame/planner routes (score/ios-score/plan/ios-plan/validate/ios-validate) content-length cap is now also DONE (#530) — a DERIVED per-route cap (300MB vision / 3GB planner), no longer deferred.]**
...
- **Track H2 follow-up:** ~~`/api/ios-score` still lacks an overall `content-length` body-size cap~~ **[DONE Run 80 / #530 — ios-score (and score/plan/ios-plan/validate/ios-validate) now enforce a pre-parse `enforceBodyLimit` before `req.json()`, per-route sized. Closed.]** Original: Reviewer B flagged it as a legitimate separate defense-in-depth item; NOT shippable disjoint from #496 that run (same file). Now landed.
