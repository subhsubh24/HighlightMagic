...
GROWTH_STATUS:
  ...
  site_gate_up: false            # HARD precondition (ROADMAP D6): pre-launch SITE GATE confirmed UP. While phase=pre_launch, EXECUTE-mode public outreach is FORBIDDEN unless this is true. Flips true only once the owner applies the gate (sets SITE_GATE_PASSWORD). Run 8 (2026-07-11): re-probed env — GROWTH_AGENT_SECRET/PROD_URL still absent, so the authenticated /api/growth/stats path cannot be tried; no new live-probe attempted this run (Run 7's curl attempt was inconclusive, proxy-layer 502, and nothing changed in-repo to warrant retrying). Stays false (fail-closed) until independently confirmed.
  validation:                    # SELF-VALIDATION (GTM_STANDARD S4) — every external source this agent depends on, re-probed against the REAL env/code this run (2026-07-11: `env` re-checked for GROWTH_AGENT_SECRET/PROD_URL/RESEND_API_KEY/KV_REST_API_URL/social tokens — all absent, same as Runs 6-7; validation-manifest.ts line numbers unchanged since last grep. Note: #441 (2026-07-10) hardened /api/growth/stats's bearer-token compare to constant-time — a security fix, not a connectivity change; GROWTH_AGENT_SECRET is still unset so the 503 unavailable behavior is unchanged)
    as_of: 2026-07-11
    sources:
  ...
  outreach:                      # STRATEGIC OUTREACH (docs/growth/OUTREACH.md) — DRAFT-ONLY, owner sends. REAL numbers; replies owner-reported, never fabricated; 0/null pre-launch
    drafted_7d: 0                # curated 1:1 Gmail drafts created for the owner in the last 7d — the one drafted Gmail draft (Sam Gutelle, Tubefilter, created 2026-06-29) is now 12 days old, outside the trailing 7d window; zero NEW drafts created this run (no verified new target this run — content-CTA honesty fix took priority, see learnings)
    owner_sent_7d: 0             # of those, how many the owner actually sent (owner-reported) — confirmed via Gmail list_drafts this run: draft still sits unsent, unedited
  ...
  next_actions:
    - "CIRCUIT BREAKER — 8 runs open: owner must connect Resend (docs/growth/CONNECT.md Step 1, ~5 min, free) + set SITE_GATE_PASSWORD to unlock execute mode. No new action needed from this agent until one of those changes — see PENDING_OPS.md gtm-connect-email / site-gate."
    - "Owner: review + send the Sam Gutelle / Tubefilter Gmail draft (search inbox drafts for 'Highlight Magic (iOS, pre-launch)'). Add full name, business address, one specific article reference first — still unsent as of 2026-07-11 (12 days old)."
