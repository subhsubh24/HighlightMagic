...
|---|---|---|---|---|
| Edit **planning** (the reasoning step) | Anthropic | `claude-sonnet-4-6` (effort=**low**) | `actions/detect.ts` | Opus 4.8→Sonnet 4.6 (2026-06-25); effort medium→low (2026-07-20, A/B-validated −76% cost, quality held 4/4). Haiku evaluated + REJECTED (empty plans on 3/4 fixtures). Env-overridable via PLANNER_MODEL / PLANNER_EFFORT for re-benching. |
...
| 2026-07-20 | Planning | Sonnet 4.6 `effort=medium` → `effort=low`; Haiku evaluated + REJECTED | effort=low HOLDS: **4/4** golden fixtures pass, themes match baseline, clip counts in-range (marginally lower). Haiku=**1/4** (empty/0-clip plans on gaming/sports/travel) → rejected. | effort=low: **−76% planner output tokens & cost** ($2.1171→$0.5007 across the 4-fixture eval; per-plan avg ~$0.53→~$0.13). Haiku's headline $0.23 is wasted on malformed output. | 3-arm golden A/B, run 29747572301 (unblocked now the Anthropic eval key is set). sonnet-medium 4/4 $2.1171 · sonnet-low 4/4 $0.5007 · haiku-low 1/4 $0.2279. Adopted sonnet-low as the env default; still overridable via PLANNER_EFFORT/PLANNER_MODEL. The planner is the creative core, so the weekly detection eval + 2-pass validation remain the guards. |
